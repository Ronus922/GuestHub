...
11. **הוראה מפורשת גוברת על שיקול דעת** - תנאי STOP, "בלי PR", "בלי מיזוג", "בלי פריסה", "אל תיגע ב-X" — הם בלתי-מותנים. גם כשההמשך נראה מתבקש, בטוח, או לטובת הבעלים: עצור, כתוב את הנימוק בדוח, והשאר את ההכרעה לבעלים. עצירה מיותרת עולה דקה; המשך שגוי עולה הרבה יותר. חריגה "מוצלחת" אינה תקדים — היא באג בתהליך.
12. **אין החלפה שקטה של החלטה של הבעלים** - כשהוראה מפורשת נראית לך שגויה, לא-אופטימלית או מיותרת: **עצור, נמק, וחכה לאישור** — לפני שאתה מבצע אלטרנטיבה. אסור לבחור באפשרות ב' כי היא "נכונה יותר", גם כשאתה צודק לגופו של עניין, וגם כשהבעלים היה מאשר אילו נשאל. הוראה שהוחלפה בשקט גוזלת מהבעלים את ההכרעה **ואת הידיעה שהייתה כאן הכרעה**. `תקדים: D159` — ההנחיה הייתה **לתקן** את 6 הפרות ה-design ב-`MyTasksScreen.tsx`; במקום זה נוצר baseline שמדלג עליהן, בלי לעצור ולשאול. הבעלים אישר בדיעבד, וזה **לא** הופך את הדרך לתקינה. הנוסח הנכון: "ביקשת א'. לדעתי ג' עדיף כי X. להמשיך עם א' או לעבור ל-ג'?" — ואז לחכות. **וכשמתעדים החלטה כזו: לא לכתוב "החלטת בעלים" על משהו שהבעלים לא הכריע בו.**
...
## שומרים וסטטוסים — שלושה חוקים שנלמדו בדרך הקשה (מ-2026-07-25)

**1. שומר נחשב שומר רק אם הוא עובר B2.**
B2 = **ניטרול סמנטי**: הפרדיקט המרכזי הופך ל-`return false` / no-op, וכל סימן
סטרוקטורלי נשאר במקומו — אותה פונקציה, אותו export, אותו טיפוס, אותה קריאה, אותו
מספר בקשות, אותה צורת מטען. **אם השומר נשאר ירוק — הוא אינו שומר.**
שומר שנשען על `grep`/`includes` על מחרוזת בקובץ מקור נכשל בהגדרה: הוא בודק צורה,
לא משמעות, והוא עיוור להעברת אותה מחרוזת לקובץ אחר.
התקן חל על **כל שומר חדש ועל כל שומר שנוגעים בו** — הרחבת חוזה היא נגיעה.
אסרשנים התנהגותיים (נקראים מ-DB או ממצב שהורץ) קודמים לסטרוקטורליים; אסרשן
סטרוקטורלי מסומן במפורש כ**חוזי**, והודעת הכישלון שלו אומרת שזה כשל חוזה ולא כשל
התנהגות. **אסור להחליש אסרשן כדי לצבוע ירוק** — חוזה שהתרחב באמת: נקוב במפורש
בהרחבה **והוסף את האסרשן המפצה** שחוסם את הסיכון שההרחבה פתחה.
*ראיה:* `scripts/check-beds24-credit-backoff.mjs` נשאר ירוק לאורך 223 שורות ראיות
עם `creditsRemaining = NULL`, כי הוא עשה grep לשם header בקובץ אחד.

**2. ג'וב שמדווח `succeeded` בלי `sentValues > 0` הוא כשל, לא הצלחה.**
כל נתיב יוצא חייב להבחין בין שלושה מצבים — **הצלחה** (נשלח ואושר),
**הצלחה-בלי-שליחה** (לא היה מה לשלוח, והסיבה מוצהרת), ו**כשל**. שני הראשונים
אינם רשאים להיראות זהים בדיעבד. **כשל קשיח נרשם תמיד** — `logChannelError`
לא-מותנה, ולא מאחורי ענף שמוקדם לו `return`.
*ראיה:* `worker.ts` החזיר `{sentValues: 0}` בשקט לחיבור שאינו drainable, ושורות
ledger מ-25/07 מראות `claimed:1, sentValues:0` לצד `claimed:1, sentValues:1` —
שתיהן `succeeded`. בנוסף, ענף השהיית קרדיטים ב-#104 בלע כשל אמיתי מאחורי
`return` מוקדם ועקף את ה-`logChannelError` שתוקן ב-#114.

**3. שינוי בסינון משיכה נכנסת מחייב מעבר על כל שער תלוי-סטטוס (D94).**
כששינוי נוגע ב**מה נמשך** — פילטר סטטוס, חלון זמן, scope של חיבור — אין להסתפק
בכך שהשער קיים בקוד. יש לאמת, לכל שער תלוי-סטטוס, שהוא **נמצא במסלול שהמשיכה
החדשה מפעילה בפועל**. שער נכון במסלול שכבר לא נקרא אינו מגן על כלום.
*ראיה:* ביטולי OTA לא הגיעו לייבוא כי הסינון החדש לא הפעיל את המסלול שהחזיק את
השער; הוא נפתר רק עם reconciliation נפרד + escape hatch מפוקח.
