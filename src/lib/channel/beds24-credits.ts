...
export type Beds24CreditSnapshot = {
  ...
  /** what the call that produced this snapshot cost (x-request-cost) */
  requestCost: number | null;
  /** false when the endpoint sent NONE of the three. "Not measured" is a real
   *  state (GET /authentication/details never meters) and must stay
   *  distinguishable from a measured 0 — which would read as "no credits left"
   *  and open the gate on a lie. */
  measured: boolean;
};
...
export function readBeds24Credits(
  ...
  const remaining = num(get(BEDS24_CREDIT_HEADERS.remaining));
  const resetsInSec = num(get(BEDS24_CREDIT_HEADERS.resetsIn));
  const requestCost = num(get(BEDS24_CREDIT_HEADERS.cost));
  return {
