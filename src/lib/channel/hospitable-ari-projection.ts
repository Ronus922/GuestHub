...
// ============================================================
// Hospitable ARI projection (D77 Phase 4) — a SIBLING of projectAri, not a
// duplicate. It exists because projectAri scopes its work through the
// Channex-owned mapping tables (channel_room_mappings.channex_room_type_id,
// channel_room_rate_mappings.channex_rate_plan_id, snapshot->>'occ_adults'),
// which are — by design, migration 044 — NEVER written for a
// provider='hospitable' connection. Hospitable maps one physical room to one
// property UUID in guesthub.channel_hospitable_property_mappings, with ONE
// designated local plan whose base-occupancy rate is the pushed price. So:
//
//   · scoping comes from channel_hospitable_property_mappings
//     (status='mapped' AND local_rate_plan_id IS NOT NULL);
//   · the occ_adults / extra-guest per-person ladder is INTENTIONALLY skipped —
//     Hospitable takes a single price per date, the plan's base-occupancy
//     nightly, so the projected rates array is exactly one entry;
...
//
// DO NOT "fix" this module by merging it back into projectAri: the two
// providers legitimately disagree about what a mapping IS. Output shape is the
// SAME AriProjection, so payload building / evidence code reads one shape.
//
...
// hospitable-ari-sync.ts.
// ============================================================

export type ProjectHospitableAriArgs = {
  tenantId: string;
  connectionId: string;
...
export async function projectHospitableAri(
  db: Sql | TransactionSql,
  args: ProjectHospitableAriArgs,
  ...
  const roomRows = await db<MappedRoomRow[]>`
    ...
    FROM guesthub.channel_hospitable_property_mappings m
  ...
  // one Hospitable property IS one physical room: a pooled sellable unit would
  ...
  // so its Hospitable property stops selling instead of selling stale prices.
  ...
  for (const room of roomRows) {
    ...
    for (const date of dates) {
      ...
      // ONE base-occupancy price — Hospitable has no per-person ladder.
