...
export async function bulkUpdateRatesAction(
  ...
  try {
    ...
    // D182: dateTo is the CHECK-OUT, so it may land one day past the horizon — the
    // last night it writes is still `latest`. Capping it at `latest` would have made
    // that night unreachable. dateFrom needs no ceiling of its own: the schema
    // enforces dateTo > dateFrom, so dateFrom <= latest follows.
    if (input.dateTo > addDays(latest, 1)) return fail(`ניתן לעדכן תעריפים עד ${latest}`);

    // dates in [from, to) — to is check-out, exclusive (D182), filtered by weekday chips (§7b)
