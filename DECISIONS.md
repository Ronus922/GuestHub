...
## D181 (עדכון קבוצתי) — טווח התאריכים הוא לילות: "עד תאריך" הוא צ'ק-אאוט, בלעדי (2026-09-10)

> **התנגשות מספור — להכרעת הבעלים.** התדריך קרא למשימה הזו D181, אבל המספר כבר נלקח
> ב-2026-09-08 לכלל `.card` / `flex: none` (‏`check:flex-card-shrink`, שתי רשומות).
> הרשומה נכתבת כאן תחת D181 **כפי שהוראה מפורשת ביקשה** ולא תחת מספר חלופי — החלפה
> שקטה של מספר החלטה היא בדיוק מה שכלל ברזל 12 אוסר. הערות הקוד, השומר וה-PR נושאים
> גם הם "D181". שינוי המספר הוא שורה אחת בכל מקום, והוא נתון להכרעת הבעלים.
...
D181 = הלילה האחרון, כולל; **אחרי** = צ'ק-אאוט, בלעדי. אין מיגרציה ואין כתיבת דאטה.
...
**גבול ידוע שלא טופל בכוונה — להכרעת הבעלים.** בדיקת האופק בשרת
(`rates/actions.ts` — `if (input.dateTo > latest)`) ותקרת הבורר (`max={maxDate}`) מנוסחות
שתיהן על `dateTo` כעל לילה. משעברנו לצ'ק-אאוט, הלילה האחרון שאפשר להגיע אליו הוא
`latest − 1` במקום `latest` — כלומר האופק לעדכון קבוצתי הצטמצם ביום אחד, בקצה של
חמש שנים קדימה. השורה לא שונתה כי היא לא הופיעה בהוראה; התיקון, אם ירצה הבעלים, הוא
לנסח את שתי הבדיקות ביחידות החדשות.
...
הסכימה **מורצת** ודוחה `dateTo === dateFrom` (בדיקה התנהגותית, לא regex); ומסנן ההזמנות
נשאר `mode="days"`. כל אחת מארבע האסרשנים הופרכה בנפרד (נטרול הפרדיקט → יציאה 1 → שחזור).
