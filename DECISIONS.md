...
2. **שדה חסר חוזר `null`, לא מחרוזת ריקה.** מתוך 14 החדרים המסומנים לאתר,
   ל-7 יש גלריה, לחלק אין `summary` ולחלק אין `size_sqm`. הצרכן מחליט מה
   להסתיר; ה-API לא ממציא ערכי ברירת מחדל.
3. **כותרת לעולם לא ריקה.** ‏`room_translations.name` → `rooms.name` →
   "<סוג חדר> <מספר>". כרטיס בלי כותרת הוא באג חזותי בצד הצרכן.
4. **הגלריה ממוינת `is_main DESC, sort_order`**, כך שהצרכן לוקח `images[0]`
   בלי להכיר את הדגל.
