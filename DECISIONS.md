...
**2.2–2.5 הורצו אחרי שרונן הסיר את שער 2.1 — והנימוק של D102 להמשך הופרך בדיוק
כמו הנימוק לעצירה.**

רונן הסיר את השער ב-2026-07-26 והורה על **2.2–2.5 בלבד, בלי דחיפה חיה**.
בוצע במלואו. **אפס כתיבה לפרודקשן, אפס שורת מיפוי, אפס קריאת HTTP ל-Beds24.**

### 2.2 — שלוש הנגזרות עוברות כל שער חוץ מאחד

נמדד מול פרודקשן: שבועי ‎−15%‎, חודשי ‎−30%‎ וגמיש ‎+5%‎ הן `is_active`,
לא-`is_archived`, **`is_visible_channels=true`**, השרשרת שלהן נפתרת אל
`fee07a5b`, ולכל אחת **15 שיוכי יחידה פעילים מתוך 16** — בדיוק כמו הבסיס.
הן אינן פגומות בשום צורה.

הפוסל היחיד הוא ה-scoping: `projectBeds24Ari` נכנס אך ורק דרך
`channel_beds24_room_mappings` עם **תוכנית אחת לכל חדר**, וכל 15 השורות נושאות
`fee07a5b`.

### ⚠️ מה ש-D102 אמר שצריך לעשות — הסכימה מסרבת לו

D102 סיכם: *"הדרך היחידה לפרסם היא לתת לכל חדר מיפוי נוסף לכל תוכנית."*
**נמדד — לא ניתן.** לא בקריאת DDL אלא בניסיון INSERT אמיתי:

| הניסיון | התוצאה |
|---|---|
| מיפוי שני לאותו חדר | **23505** · `..._connection_id_room_id_key` |
| מיפוי נוסף אל אותו חדר Beds24 | **23505** · `..._connection_id_beds24_property__key` |
...
לכן פרסום שלוש התוכניות דורש **60 חדרי Beds24 במקום 15**, או אינטגרציה חדשה מול
ציר תעריפים אחר של הספק — שאינה קיימת בקוד. זו עבודה חדשה, לא הפעלת קיים.

### 2.4 + 2.5 — הדריי-ראן עבר על 84 תאים

`check:beds24-derived-dryrun` מקמפל את **הפרויקציה והבנאי האמיתיים** מהעץ שהוא חי
בו, ומריץ אותם על ה-DB המבודד (:5433) בטרנזקציה שמתגלגלת לאחור, עם צורת הנתונים
של פרודקשן (610 / 750 / 700). **`beds24-ari-sync.ts` לא מיובא.**

חודשי ‎−30%‎ → 427 / 525 / 490 · שבועי ‎−15%‎ → 518.5 / 637.5 / 595 ·
גמיש ‎+5%‎ → 640.5 / 787.5 / 735.

בכל 84 התאים **מקומי = נשלח = צפוי**, כשהצפוי מחושב בחשבון פשוט ולא ע"י הקוד
הנבדק. ביקורת שלילית: בלי מחיר בסיס ליחידה ⇒ `NO_PRICE_FOR_DATE`, `numAvail:0`,
**ובלי `price1`** — fail-closed, בלי אפס ובלי ניחוש.

**2.3 מאושר סופית:** הנגזרת מחושבת מהבסיס בזמן הבנייה. D102 הוכיח זאת ברמת
הפותר בלבד; עכשיו זה מוכח מקצה לקצה עד ה-payload.

### B2 — שתי מוטציות סמנטיות, שתיהן נתפסו

היפוך סימן ההתאמה ב-`applyPlanAdjustment` ⇒ `701.5 ≠ 518.5`. פרויקציה ששולחת את
מחיר הבסיס תחת תוכנית נגזרת ⇒ `610 ≠ 518.5`. השנייה היא בדיוק הנסיגה שהשומר קיים
בשבילה. `check:guard-roots` עובר על השומר החדש (84 סקריפטים).

### פגם שהתגלה אגב: שרשרת המיגרציות אינה idempotent

הרצה שנייה על סכימה שכבר עברה מיגרציה **מתה**: מיגרציה 005 בונה אינדקס חלקי על
`channex_room_type_id`, ומיגרציה 054 שינתה את שמו ל-`external_room_type_id`.
מ-אפס השרשרת מתנגנת נקי; על סכימה קיימת — לא. **לכן כל שומר מגובה-DB חייב
`DROP SCHEMA` לפני ההרצה**, וזהו לפחות חלק מהשורש של "15 השומרים תלויי-הסביבה"
ב-D101. השומר החדש עושה זאת במפורש ומתעד למה. **לא תוקן במקור** — תיקון 005
דורש הכרעה נפרדת.

### פגם תיעוד: D102 מצטט ראיות שאיש אינו יכול לראות

`ref/proof/pricing-origin.txt` ו-`pricing-inheritance-resolver.txt` שמצוטטים
ב-D102 **אינם בריפו ולעולם לא יהיו**: `/ref/` הוא gitignored לפי D87 (הריפו
פומבי). לכן המספרים עצמם הוכנסו הפעם לתוך `docs/PRICING_MODEL.md`, והשומר החדש
לא נשען על התיקייה הזו — הוא יוצר אותה בעצמו ועובר גם בלעדיה.

### לא בוצע — 2.6, ולהכרעת רונן

**דחיפה חיה לא בוצעה ולא תבוצע בלי הוראה נפרדת.**

1. **האם למכור את שלוש התוכניות בכלל** — אחרי D100 נשמרות שהיות של חודשים עד
   שנה במחיר לילה מלא. זה הפער המסחרי.
2. **אם כן — באיזה מבנה:** 60 חדרי Beds24, או אינטגרציה מול ציר תעריפים אחר.
3. **המחיר 750 ל-1318** — פתוח מ-D99/F.

פירוט: `docs/PRICING_MODEL.md`.
