...
- **אין CI.** אף אחד מ-74 השומרים אינו רץ אוטומטית.
  > **בוטל ב-04/09/2026** — CI הוקם ופעיל (`typecheck`, `lint`, `suite`), נדרש כ-status checks על `main`. ראה `.github/workflows/ci.yml`.
...
## D172 — נושא המייל בשליחה ידנית מתרנדר דרך renderer התקשורת; משתנה לא מוכר חוסם ולא נשלח מילולית (2026-09-04)

**הראיה.** ‏outbound_messages ‏`6518a028`, ‏2026-09-04 20:46Z, הזמנה 1159: המייל
יצא ל-Gmail עם הנושא המילולי `אישור הזמנה {{reservation.number}} · {{property.name}}`.
‏`sendBookingEmailAction` רינדר את הנושא דרך `renderTemplate` הישן
(`lib/messaging/templates.ts`), שה-regex שלו תופס רק מפתחות `{{snake_case}}`;
מפתח עם נקודה אינו נתפס ולכן נשאר כמות שהוא. ההערה בקוד ההוא, "placeholder תועה
לעולם לא נשלח מילולית", הייתה נכונה רק לדקדוק הישן.

**ההכרעה (בעלים).** נושא בשליחה ידנית מתרנדר דרך אותו renderer שמרנדר את
תוכן האוטומציות, ‏`renderTemplateString` מ-`lib/communications/renderer.ts`,
ומפתח לא מוכר אינו יוצא מילולית.

**המימוש.** מודול חדש `lib/messaging/render-manual.ts` עם `renderManualSubject`:
מעבר ראשון ב-renderer הישן על מפתחות `{{snake_case}}` (השבבים של הקומפוזר
ותבניות ישנות — אחת משתי תבניות המייל הפעילות עדיין בדקדוק הזה), ואז
‏`renderTemplateString` על `{{group.key}}` עם ה-context של ההזמנה, שנבנה
ב-`reservationRenderContext` החדש ב-`automation.ts` דרך אותו `buildRenderContext`
של ה-worker. שני הדקדוקים זרים זה לזה (הישן לעולם לא תופס נקודה, החדש דורש
אחת), ולכן אין רינדור כפול וסדר המעברים אינו משנה. מפתח מנוקד לא מוכר
מרוקן **וגם** חוסם את השליחה עם שם המשתנה (סמנטיקת D115, ראיה לפי D112);
מפתח ישן לא מוכר ממשיך להתרוקן בשקט, כפי שהיה. התצוגה המקדימה בקומפוזר
עברה לאותה שרשרת, וכפתור השליחה נחסם כשהנושא חסום, כדי שהתצוגה לא תראה
מה שהשליחה לא תשלח. אם לא ניתן להרכיב את ה-context (פרופיל עסק / לוח שעות),
השליחה מסורבת עם סיבה ולא נשלחת עם token גולמי. שומר: `check:manual-send-subject`
(חיווט סטטי + שישה תרחישים על המודול המקומפל, בלי DB).

**מה נשאר כפי שהיה, בכוונה, וממתין להכרעה.** גוף ההודעה בשליחה ידנית,
במייל וב-WhatsApp, עדיין מתרנדר ב-renderer הישן. נמדד: אצל 4 תבניות
WhatsApp פעילות, 2 משתמשות במפתחות מנוקדים בגוף — שליחה ידנית שלהן תשלח
אותם מילולית. ההרחבה לגוף היא שינוי התנהגות נפרד ולא בוצעה כאן.

**תוספת 2026-09-05 — הגוף עובר באותה שרשרת (הכרעת בעלים, אחרי פריסת #229).**
גוף ההודעה בשליחה ידנית, במייל וב-WhatsApp, מתרנדר עכשיו דרך אותה שרשרת כמו
הנושא: `renderManualText` ב-`lib/messaging/render-manual.ts` (השם הכללי החליף את
`renderManualSubject`; אותו קוד, אותם כללים). מפתח מנוקד לא מוכר בגוף מרוקן
**וחוסם** את השליחה עם שם המשתנה בעברית ("תוכן ההודעה מכיל משתנה שלא ניתן לשלוח
— משתנה לא מוכר: …"); ערך אופציונלי חסר מתרנדר ריק ואינו חוסם (D115); מפתח ישן
לא מוכר מתרוקן בשקט כמו קודם. ב-WhatsApp, שלא היה לו מסלול V2 בכלל, ה-context של
ההזמנה נבנה עכשיו גם שם, ואי-יכולת להרכיב אותו מסרבת בשם הסיבה. התצוגה המקדימה
בקומפוזר מרנדרת את הגוף באותה שרשרת בשני הערוצים וחוסמת את הכפתור כשהגוף חסום.
נמדד לפני השינוי: 4 תבניות WhatsApp פעילות, 2 מהן עם מפתחות מנוקדים בגוף
("הודעה לעצמי על הזמנה", "וואטצאפ אישור הזמנה") — כל 15 המפתחות שהן משתמשות בהן
קיימים בקטלוג `COMMUNICATION_VARIABLES`. השומר `check:manual-send-subject` שונה
ל-`check:manual-send-render`: אותם 6 תרחישי נושא + 5 תרחישי גוף, ובהם גופי שתי
התבניות האמיתיות כ-fixtures מילוליים (בלי DB), כולל שמירת שורות.

## D173 — כרטיס "ערוצי שליחה" בדשבורד קורא את `outbound_messages`, לא את בדיקת החיבור; מייל לבעלים פעם אחת לרצף של 3 כישלונות (2026-09-05)

**הראיה.** מ-02/09 עד 05/09/2026 כל שליחת WhatsApp נכשלה `green_400`
("Instance account is expired. Renew your instance from personal area.") — 9 שורות:
שלושה ריצות אוטומציה (הזמנות 1156, 1158), שליחה ידנית של הבעלים (04/09 21:02Z)
ושליחה מבוקרת אחת (05/09 09:35Z). ההצלחה האחרונה לפניהן: 20/08 18:15Z. כל אותו זמן
`messaging_provider_connections.status` הציג "מחובר" מבדיקה של 30/07, ואף מסך לא
האדים. בדיקת חיבור היא עובדה על הרגע שבו נלחץ הכפתור; יומן השליחות הוא עובדה על
כל הודעה מאז. אחרי חידוש המנוי (05/09) השליחה הראשונה עברה (`16e19128`, 09:45:58Z,
GREEN-API ‏200, ‏idMessage ‏3EB0E8928C5F0B349E5383).

**ההכרעה (בעלים).** כרטיס KPI "ערוצי שליחה" בדשבורד, מייל ו-WhatsApp, שמצבו נגזר
מ-`outbound_messages` ולא מבדיקת החיבור: ירוק אם השליחה האחרונה הצליחה, אדום אם
נכשלה (עם `error_code` מתורגם לעברית ומספר הכישלונות הרצופים), אפור אם אין שליחות
7 ימים. לחיצה על שורה אדומה פותחת את כרטיס ההגדרות של הערוץ. בנוסף מייל לבעלים על
3 כישלונות רצופים באותו ערוץ — פעם אחת לרצף, לא בכל כישלון.

**המימוש.** הכלל טהור ב-`lib/messaging/channel-health.ts` (בלי imports, בלי שעון —
`nowMs` מהקורא, כדוגמת D133), והשאילתה היחידה שמזינה אותו ב-`channel-health-db.ts`;
הכרטיס (`DashboardScreen` → `ChannelsKpi`) והמייל קוראים דרך אותו loader ולכן לא
יכולים לחלוק. **מה נחשב שליחה:** רק שורות שהגיעו לספק — הצלחה = submitted/sent/
delivered/read; כישלון = `failed` שסיבתו הספק (קוד `green_*`/`gmail_*`/`twilio_*`,
adapter שזרק, ערוץ לא מחובר/לא ניתן לפענוח, או קטגוריית `provider_*` של ה-worker).
כל השאר — נמען שגוי, מספר בלי WhatsApp (`undelivered`), snapshot לא שלם, תוצאה לא
ודאית, `skipped`/`cancelled` — **מתעלמים ממנו**: זו עובדה על הנמען, לא על הצינור, והיא
לא תצבע את הערוץ באדום ולא תשלח מייל. **רגע השליחה** הוא `COALESCE(submitted_at,
updated_at)` ולא `created_at`: שורת worker נוצרת בזמן התיור ועלולה להיכשל שעה של
retries אחר כך. **"פעם אחת"** הוא עובדת DB, לא זיכרון תהליך: טבלה חדשה
`messaging_channel_alerts` (מיגרציה 087) עם `UNIQUE (tenant_id, channel, streak_key)`,
כאשר `streak_key` = מזהה ההצלחה האחרונה שלפני הרצף (או `'none'`); ‏`INSERT … ON
CONFLICT DO NOTHING` הוא הנעילה — מי שהניח את השורה שולח, כל כישלון נוסף באותו רצף
מוצא אותה ועוצר; הצלחה חדשה פותחת מפתח חדש ולכן התקלה הבאה תדווח שוב. שני מסלולי
הכתיבה של כישלון נעוצים בה: `service.ts` (ידני) ו-`delivery.ts markFailed` (worker,
רק כישלון סופי). המייל יוצא דרך חיבור ה-Gmail של הדייר אל
`failure_notification.email` אם הוגדר, אחרת אל `owner_notification_emails`, ואינו
נרשם ב-`outbound_messages` — התראה על הצינור לא תהפוך לנקודת נתון על הצינור. תוצאת
המייל נרשמת על שורת ההתראה (`email_status`), כולל המקרה שבו ערוץ המייל עצמו הוא
שנכשל — נרשם, לא מנוסה שוב; הכרטיס הוא המשטח שאינו תלוי בערוץ עובד. שומר:
`check:channel-health` (כלל טהור: 13 קבוצות תרחישים כולל שחזור רצף 02–05/09 האמיתי,
מוטנט B2, וחיווט סטטי של הכרטיס, שני ה-hooks, הנעילה והמיגרציה).

**מה לא נעשה, בכוונה.** הכרטיס אינו מחליף את בדיקת החיבור במסך ההגדרות ואינו משנה
את `messaging_provider_connections`; 9 הכישלונות ההיסטוריים לא מקבלים התראה בדיעבד
(ברגע הפריסה הרצף כבר נשבר ע"י ההצלחה של 09:45Z). ‏`green-api.ts` עדיין זורק את גוף
...
## D175 — מסך הגדרות: טאב "פרופיל העסק" לפי העיצוב המאושר, וגריד ההגדרות (ניווט 256px + תוכן) (2026-09-05)

**ההוראה (בעלים).** "הוראות לקלוד קוד — מסך הגדרות: פרופיל העסק": ליישם 1:1 את
העיצוב המאושר `הגדרות - פרופיל העסק.dc.html` בטאב הקיים — באנר קומפקטי, כרטיס
מוכנות מפוצל לשתי עמודות עם צ'יפ סטטוס לכל עמודה, כרטיסים אחידים (אייקון + כותרת +
תת-כותרת), כפתור שמירה ראשי כחול אחרי כרטיסי הזהות, סיכום מיקום בגריד 4 עמודות,
"מיקום ידני מתקדם" כאקורדיון סגור, ו"שמירת מיקום" בתחתית הכרטיס; ניווט הגדרות 256px
דביק, פריטים 40px/‏radius 11, קריסה לעמודה אחת מתחת ל-1120px. "אין שינוי ב-API קיים".
קובץ ה-`.dc.html` עצמו לא נמצא בריפו, בדיסק או בארטיפקטים בזמן היישום — המימוש הוא
לפי מפרט ההוראות (ערכים, טוקנים, מבנה), שהוא מפורט עד רמת הפיקסל. (D174 תפוס בענף
`fix/d173-paid-default-zero` שטרם מוזג, ולכן הפרק הזה הוא D175.)

**המימוש.** קובץ partial חדש `app/styles/business-profile.css` (קידומות `sg-` לגריד
ההגדרות, `bp-` לטאב), `SettingsShell` על `.sg-grid` (256px + `minmax(0,1fr)`, gap 18,
מקס 1380, ניווט `sticky top:18px`), `SettingsCard` המשותף קיבל `subtitle` אופציונלי
וקלאס `card-ic` על ריבוע האייקון (טאבים אחרים לא השתנו), ארבע ליגטורות חדשות ב-`Icon`
(`verified`, `photo_camera`, `open_in_new`, `pan_tool_alt` — כולן קיימות ב-woff2
המצורף, נבדק). `BusinessProfileSection` נכתב מחדש סביב אותה לוגיקה (reload, שמירה,
לוגו); `LocationPicker` שומר את כל מחזור ה-SDK (D61/D62) ומחליף רק את התצוגה. שני
...
במנוחה (‎§5 = 44px לבן עם מסגרת); ריבוע אייקון 38px/‏radius 11 ושורת ניווט radius 11
ומפה radius 14 (‎§1 = ‏{16,12,8,7}); צ'יפ סטטוס pill ‏12px/‏3px 10px/‏999 (‎§3 = אנטומיה
אחת 28px/‏radius 8); צבעי ההפניה שאינם טוקנים (‎#EEF0F5, ‎#FBFCFE, ‎#E7F6EC/‎#15803D,
‎#FEF3E2/‎#B45309, ‎#E11D48, ‎#B4231F/‎#FDECEC, ‎#C2C8D6, ‎#2C3A7E, ‎#F5F7FB); תוויות
קבוצה בניווט עם `letter-spacing:.05em` — **הופך** הערת קוד קודמת "never letter-spaced
(audit F-4)" שמקורה לא נמצא בתיעוד; שורת ניווט 40px — ועל מצביע גס 44px (כלל ברזל 6).

**הכרעות שלי במקומות שבהם המפרט שתק או התנגש עם כלל קיים — ממתינות לרונן, אינן
"החלטת בעלים" (כלל ברזל 12):**
1. **המפה היא ה-JS SDK עם סמן נגרר, לא `iframe` embed** (340px/‏radius 14 כן מיושמים):
   המפרט עצמו דורש "גרירת סמן לא נשמרת עד שמירה", ו-iframe אינו מאפשר גרירה. ה-iframe
   במוקאפ הוא placeholder.
2. **הטוסט הוא הטוסט המערכתי הקיים** (‎§9: ink, radius 12, check_circle ירוק בהיר,
   ממורכז-תחתון) — זהה לתיאור, אבל 2.8 שניות ולא ~2.2; "מבנה יחיד לכל המערכת".
3. **אזור זמן באקורדיון = select מושבת** המציג את הקנוני של הדייר: אין endpoint לכתיבתו
   (`profile.ts`: "never overwritten"), והמפרט אסר שינוי API.
4. **האקורדיון גלוי לכל בעל `settings.edit`** (מיקוד + אזור זמן); **שדות הקואורדינטות
   רק ל-super_admin** (‎§6 + השומר). ההסתרה המלאה הייתה גוזלת ממנהל רגיל את היכולת
   היחידה להזין מיקוד — ש-Google לא תמיד מחזיר.
5. **צ'קבוקס האישור ואזהרת "Google Maps הוא המקור המועדף" מופיעים רק כשהוקלדו
   קואורדינטות** — השרת דורש `confirmed` לדריסה ידנית (‎§6) והשומר דורש `manualConfirm`;
   כשמזינים מיקוד בלבד האקורדיון נראה בדיוק כמו במוקאפ.
6. **"שמירת מיקום" אחת** מכריעה לפי מה שממתין: מקום שנבחר/סמן שנגרר → דריסה ידנית →
   מיקוד לבד (`saveBusinessProfileAction`). מיקוד שמוקלד בזמן שיש בחירה ממתינה מתמזג
   לתוכה, כדי שלא יהיו שני מסלולי שמירה לאותה לחיצה.
7. **סרגל "ממתין לשמירה"** (כתובת/עיר/מיקוד/קואורדינטות + ביטול) — המפרט לא הגדיר מצב
   pending; הסיכום נשאר מהפרופיל השמור בלבד (Maps שנפל לא מרוקן אותו — שומר קיים).
8. **מובייל מתחת ל-768:** ה-select הקומפקטי של המעבר המובייל (D145/D146) נשאר; בין 768
   ל-1120 הניווט עובר מעל התוכן בעמודה אחת כפי שהמפרט קובע. כרטיס ניווט של ~15 שורות
   מעל התוכן בטלפון היה דוחף כל טאב מסך שלם למטה.
9. **JetBrains Mono לא נטען** (‎§2/D2: Assistant בלבד; טעינת פונט נוספת = שינוי ב-layout
   לכל האפליקציה) — מחסנית mono מערכתית שמעדיפה JetBrains Mono אם מותקן.
10. **סוגי מקום האירוח = 10 הקיימים** ב-`PROPERTY_TYPES`; "מלון דירות" מרשימת המוקאפ
    אינו בסט הסגור ולא נוסף (שינוי API/דומיין).
11. **תתי-הכותרות של ארבעת הכרטיסים הן ניסוח שלי** — המפרט ציין "תת-כותרת" בלי טקסט.
12. אייקון ניווט 19px ואייקון רשימה 18px נצמדים ל-20/17 (‎§10 — `<Icon>` מצמיד).
13. חיפוש הכתובת הוא `<gmp-place-autocomplete>` של Google (shadow DOM): מראה השדה
    מורכב סביבו (אייקון search שלנו + משטח אחד), ה-`×` הפנימי של Google הוא הניקוי;
    משתני `--gmp-mat-*` מוגדרים ולא ניתן לאמת אותם בלי דפדפן.

**אימות.** `tsc --noEmit` 0 · `eslint` 0 על הקבצים · `check:design` ✔ · `check:responsive`
...
### D175 — סגירת פריטים 2, 8, 9, 11 (הכרעות בעלים 2026-09-13)

ארבעת הפריטים שנותרו פתוחים ברשימת "הכרעות שלי" הוכרעו ע"י הבעלים; המימוש ב-PR
`chore(D175)` (ענף `fix/d175-close-8-9-11`), ללא מיגרציה וללא שינוי API.

**פריט 2 — משך הטוסט: נשאר 2.8 שניות של הטוסט המערכתי.** אין שינוי בקוד (הוכרע כך כבר
ב-D177 §3; GUIDELINES §9 "מבנה יחיד לכל המערכת").

**פריט 8 — ניווט ההגדרות בין 768 ל-1120: רצועת טאבים אופקית קומפקטית מתחת לכותרת
(אפשרות ג), לא כרטיס ניווט של ~15 שורות מעל התוכן.** מתחת ל-768 ה-select הקומפקטי נשאר;
מ-1120 ניווט הצד נשאר. המימוש הוא CSS על אותו DOM (`business-profile.css`):

```css
@media (min-width: 768px) and (max-width: 1119.98px) { … }
```

בטווח הזה `.sg-nav` הופך ל-`flex-direction: row; flex-wrap: wrap; gap: 2px 4px`, הקבוצות
(`.sg-grp`) והרשימות (`.sg-list`) הופכות ל-`display: contents` כך שכל הפריטים הם שורה
אחת נשברת, תוויות הקבוצה מוסתרות (לרצועת טאבים אין כאלה), הפריט שומר 40px (44 במצביע
גס) ואת משטח `.on` (`--brand-soft`), ופס ה-3px של הפריט הפעיל עובר לקצה התחתון
(`inset-block: auto 0; inset-inline: 10px`). מעטפת הכרטיס (padding/מסגרת/צל/רקע)
מנוטרלת בטווח הזה: תשעת הטאבים צריכים ~1,370px, כלומר שתי שורות ב-900, ושתי שורות של
40px חייבות להשאיר את התוכן בתוך 120px מהכותרת — עם padding של כרטיס זה בלתי אפשרי
(‏18 + 2×40 + 2 + 18 = 118 בלי מעטפת). ה-`ul` קיבל קלאס `sg-list` במקום `flex flex-col
gap-0.5`: שכבת utilities של Tailwind גוברת על `@layer components`, ולכן `flex-col` לא
ניתן לדריסה מתוך ה-partial. `overflow-x: auto` נשאר לגלילה אם טאב בודד רחב מהרצועה.
השומר `check:check-in-check-out` (‏`md:hidden` / `card sg-nav hidden md:flex`) לא השתנה
ונשאר ירוק.

**פריט 9 — JetBrains Mono אינו נטען (אפשרות א).** מחסנית המונו המערכתית נשארת
(`ui-monospace, "JetBrains Mono", "SFMono-Regular", Menlo, Consolas, monospace` ב-`.bp-mono`
וב-rate-cell-panel); GUIDELINES §2 תוקן לתאר את המחסנית בפועל במקום פונט שמעולם לא נטען.

**פריט 11 — ארבע תתי-הכותרות מאושרות כפי שהן, עם תיקון אחד (אפשרות ב).** הרביעית
("מיקום"): "כתובת, קואורדינטות ואזור זמן של הנכס. מקור אחד לכל הערוצים" — הקו המפריד
הוחלף בנקודה. שלוש האחרות ללא שינוי.

**אימות (CDP על ה-CSS האמיתי, מקומפל ב-`@tailwindcss/postcss` של הפרויקט לסקראץ'; ה-DOM
של `SettingsShell` הועתק 1:1 ל-fixture סטטי עם 9 הסעיפים של super_admin, Assistant
ו-Material Symbols נטענים).** 900×800: ה-select מוסתר, `.sg-nav` הוא `flex row wrap`
ללא רקע/מסגרת/צל/padding, 9 פריטים ב-**2 שורות**, גובה הרצועה 82px, כל פריט ≥40px,
תוויות קבוצה 0 גלויות, פס פעיל 3px בתחתית, **התוכן מתחיל 118px מתחת לכותרת** (≤120),
ואין כרטיס ניווט מעל התוכן. 390×844 (מגע): ה-select גלוי, `.sg-nav` = `display: none`.
1280×800: ניווט צד 256px `sticky`, עמודה, 9 שורות, select מוסתר. צילומים:
`/tmp/d175-390.png`, `/tmp/d175-900.png`, `/tmp/d175-1280.png`. `tsc --noEmit` 0, eslint 0,
`check:check-in-check-out` ✔ (ללא שינוי), `check:flex-card-shrink` ✔, `check:design` ✔,
`check:responsive` ✔.

## D177 — מגירת "מצב מכירה ליום" ב-/rates לפי העיצוב המאושר: טיוטה מקומית, שמירה אחת, סטטוס חי, טוסט מערכתי (2026-09-06)

**ההוראה (בעלים).** "הוראות לקלוד קוד — עיצוב מחדש: חלון 'מצב מכירה ליום' (Drawer)":
ליישם 1:1 את `מצב מכירה ליום.dc.html` בחלון הצד שנפתח מתא יום של חדר — פס כותרת תקני,
צ'יפ סטטוס אחד + משפט הסבר שמתעדכן חי לפני שמירה, בורר מכירה צבעוני (ירוק/אדום עם
check), סטפרים `[−] ערך [+]` ב-LTR עם "—" לריק, מתגים אמיתיים עם תיאור משנה, שדה מחיר
עם ₪ וכפתור "שמירת מחיר" צמוד (disabled כשאין שינוי), פוטר קבוע עם מחוון "שינויים שלא
נשמרו" ו"שמירת שינויים" בקריאת API אחת. "שינוי תצוגה בלבד — אין שינוי ב-API ובלוגיקה".
קובץ ה-`.dc.html` לא נמצא בדיסק (כמו ב-D175) — המימוש לפי המפרט הכתוב וצילום המסך של
העיצוב שצורף. (D176 תפוס בענף `docs/d176-phantom-cash-void`, ולכן הפרק הזה הוא D177.)

**ארבע הכרעות בעלים שנשאלו לפני המימוש (כלל ברזל 12), 2026-09-06:** (1) **בלי
"(CTA)"/"(CTD)"** בתוויות "סגור לכניסה"/"סגור לעזיבה" — השומר `check:cell-mark-ladder`
§16 (קומיט 916f676) נשאר כמו שהוא; (2) **פס כותרת קנוני** — ריבוע אייקון 40 וכפתור
סגירה 36 של `SidePanel` (§7, D125), לא 42/38 של המפרט; (3) **המערכת גוברת בשלושה
כללים גלובליים** — טוסט 2.8 שנ׳ ולא ~2.2 (§9), בלי טעינת JetBrains Mono (mono stack
כמו D175 §9), קווי הפרדה `var(--line)` ולא ‎#EEF0F5 (§12.2); (4) **שורות הקישור**
להזמנה/חסימה והודעות מצב החדר מתחת לאריחי המלאי **נשארות** (המפרט לא הציג אותן).

**המימוש.** המגירה היא `CellDetailPanel` של **‎/rates** (נפתחת מתא מחיר; ביומן אין
מגירת-יום — הכרטיס הצף שלו נמחק בכוונה, `check-cell-mark-ladder.mjs`). הקובץ נכתב מחדש:
מעטפת דקה + `CellDetailDrawer` עם `key` של (יחידה, תאריך) — הטיוטה מתאפסת בכל פתיחה,
לא בכל רענון שרת. מודל הטיוטה הוא overlay של עריכות המשתמש מעל התא השמור, ולוגיקתו
טהורה ב-`src/lib/rates/cell-draft.ts` (אפס imports): `draftPatch` שולח **רק שדות ששונו,
בקריאה אחת** ל-`upsertRateCellAction` (הסכמה `rateCellPatchSchema` קיבלה כמה שדות
ב-patch אחד מאז ומעולם — אפס שינוי בשרת); `pruneEdits` מוחק עריכות שנחתו אחרי
`router.refresh()` (הרענון רץ ב-`useTransition`, כך ש-busy נמשך עד שהתא הטרי נוחת ואין
הבהוב של ערכים ישנים); `liveSellable` גוזר את הצ'יפ הסופי **חי** (הטיוטה מחליפה את
הפסק המסחרי, כל ציר אחר נקרא כפי שנשמר — יום תפוס נשאר "לא זמין למכירה"); `stepStay`
(null ⇄ 1 ברצפה, תקרה 3650, 0 לא ניתן להשגה); `outboundSummary` בונה את סיכום
ה-ARI מהטיוטה. מחיר: שדה ספרות בלבד (`type="text" inputMode="numeric"`, מסונן ב-onChange)
עם "שמירת מחיר" נפרד, disabled כשהערך שווה לשמור או ריק. `window.alert` הוחלף בטוסט
המערכתי (`sonner`, `.gh-toast`). partial חדש `app/styles/rate-cell-panel.css` (קידומת
`rc-`), **לא-משוכב בכוונה**: `SidePanel` פולט `p-0` (utility) על גוף המגירה, ורק כלל
לא-משוכב מנצח utility (D148/§16; אותו מנגנון כמו `group-update.css`); צורך את
`.card/.card-hd/.card-bd/.btn*/.icon-btn/.field-input/.tile*/.dw-*` ולא מצהיר אותם מחדש.
שלוש ליגטורות ב-`Icon` (`event_available`, `flag`, `radio_button_unchecked` — כולן
ב-woff2 המצורף, נבדק ב-fontTools), `HEBREW_DAY_NAMES` ב-`dates.ts` (תת-הכותרת מציינת
"יום חמישי"). שומר חדש **`check:rate-cell-panel`** מקמפל את המודול הטהור ומריץ אותו
(סטפר, patch, קטגוריות הפוטר, prune, סטטוס חי, סיכום) ובודק סטטית את החיווט (בלי
`window.alert`, `sonner`, כלי כתיבה אחד, `useTransition`, `key` פר תא, partial לא-משוכב
ומיובא לפני `responsive.css`, הליגטורות). mutation-tests: `window.alert` חוזר → אדום;
`stepStay` מחזיר 0 → אדום; שחזור בייט-זהה.

**ערכים שהעיצוב המאושר קובע מחוץ לסטים הסגורים — כולם עם `ds-allow: מצב מכירה
ליום.dc.html` (grep אחד מוצא את 22-ם):** גבהים 40 (צ'יפ הסטטוס), 52 (בורר המכירה), 48
(שדה המחיר), 38 (כפתורי הסטפר); `font-size` 16 לערך הסטפר (§2 "אין 16"); `border-radius`
13 לבורר (§1); צבעי ההפניה שאינם טוקנים (‎#E7F6EC/‎#15803D, ‎#FDECEF/‎#BE123C, ‎#F2FBF5,
‎#FDF1F4, ‎#E11D48 — שאינו `--danger` ‎#E5484D, ‎#EFF2FB, ‎#C7CEE8, ‎#B45309, ‎#D3D9E5);
שתי טבעות הבחירה `0 0 0 3px rgba(…,.1)`.

**הכרעות שלי במקומות שבהם המפרט שתק — ממתינות לרונן, אינן "החלטת בעלים" (כלל ברזל 12):**
1. **יחידה מאוגדת (pooled):** כותרת "מאגר {code}" במקום "חדר {code}"; תת-הכותרת שומרת
   "· מאגר N". המפרט מציג חדר בודד בלבד.
2. **סטטוס סופי כשחסום מסיבה לא-מסחרית** (תפוס/חסום/מושבת/בלי מחיר): המשפט הוא
   `SELL_REASON_TEXT` של הקוד הלא-מסחרי הראשון — אוצר המילים הקיים, בלי ניתוח מחרוזות.
3. **hint "מחיר אפקטיבי":** "(מוגדר)" כשמפורש, "(בסיס ₪X)" כשמוסק — כמו קודם.
4. **מחיר:** ספרות בלבד ⇒ שלמים; ניקוי חזרה למחיר הבסיס אינו אפשרי מהמגירה (המפרט:
   "ריק לא ניתן לשמירה"; העורך המוטבע בלוח עדיין יכול). טוסט "המחיר נשמר ליום dd/mm/yyyy".
5. **סיכום הסנכרון:** ה-₪ הוא המחיר **השמור** (`outboundRestrictions.rate` — טיוטת השדה
   עוד לא "מה שיישלח"), והסיכום **כולל "מ׳ הגעה"** אף שרשימת המפרט לא מונה אותו: הטקסט
   הקודם כלל אותו, והשמטה מסתירה הגבלה אמיתית.
6. **מצבי קריאה בלבד** (אין הרשאה / אין תוכנית פעילה / תאריך שעבר): שלושת המשפטים
   הקיימים במקום הפקדים; הפוטר זהה, הראשי disabled.
7. **גוני אריחי הסנכרון מעבר לשניים שבמפרט:** `processing`→כתום, `failed`→אדום,
   `not_connected`→ניטרלי; "לא ממופה"→אדום.
8. **"זמין" ירוק רק כש->0; "מצב חדר" ירוק רק כש"פעיל";** ערכי טקסט ארוכים ("מושבת (לא
   פעיל)") נשברים לשורות בתוך האריח (`overflow-wrap: anywhere`).
9. **‎#C7CEE8 ל-disabled חל על שני הכפתורים הראשיים במגירה** (המפרט מציין רק "שמירת
   מחיר"; הדמו מציג את שניהם באותו גוון).
10. **סגירה עם שינויים: שקטה** (לפי המפרט; אין דיאלוג גלובלי — דפוס `confirmDiscard` של
    `EditReservationPanel` קיים אם יידרש).
11. **רספונסיב מעבר למפרט:** שורות המתג זו לצד זו ברוחב (auto-fit 240px) ומוערמות בצר;
    אריחי השהייה 2+1 בטלפון; אריחי המלאי 6 בשורה מרוחב 700, 3×2 בטלפון, 2 עמודות
    ב-320; ריפוד גוף 14px בטלפון.
12. **אנימציית המגירה:** ברירת המחדל של `SidePanel` (1.2s) — המפרט שתק.
13. **[+] בסטפר עוצר ב-3650** (תקרת הסכמה) בשקט.
14. **breakpoint הרוחב 1101px** (המפרט: "≤1100 — מלא"); הרצפה כתובה `min(700px, 100%)`
    כדי לא ליפול ב-`check:responsive` §8 (תקדים העורך V2).

**אימות.** `tsc --noEmit` 0 · `eslint` 0 שגיאות (3 אזהרות ישנות ב-`ref/audit`, לא בקוד) ·
`check:design` ✔ (0 הפרות חדשות) · `check:responsive` 20/20 · `check:cell-mark-ladder`
18/18 · `check:rates-ui` 68 · `check:calendar-ui` ✔ · `check:guard-roots` ✔ ·
`check:rate-cell-panel` 8/8 + שני mutation-tests אדומים. **לא אומת:** תצוגה בדפדפן — אין
build בעץ הפרודקשן (D147); אימות ויזואלי אחרי deploy ב-1440/1000/390 (`hasTouch:true`,
D148), עם rollback מוכן: פתיחה → שינוי מכירה → הצ'יפ מתהפך חי → שמירה → טוסט →
המחוון נעלם; מחיר שווה לשמור ⇒ כפתור disabled; סטפר ריק ⇒ [−] disabled; Esc/רקע/ביטול
סוגרים בלי שמירה; חדר תפוס ⇒ "לא זמין למכירה" + שורת הקישור להזמנה.

### D177 — תוספת מיזוג, פריסה, אימות ופגם החיתוך (2026-09-06)

**מיזוג ופריסה.** ‏#240 מוזג ל-main ב-merge commit `dd9feb9b` (21:40Z; ענף ה-feature נמחק
ב-remote ובמקומי — הריפו מאפשר את שלוש השיטות בלי מדיניות כתובה, וה-PRs הרב-קומיטיים
הקודמים ‏#234–#236 מוזגו כך). נפרס דרך `PROD_DEPLOY_OK=1 npm run deploy:prod`: ‏0 מיגרציות
ממתינות (89 קבצים), build `g_m9Zh0Zcb-T8kd5JJb1y`, שני תהליכי PM2 online מ-`/var/www/guesthub`,
‏`/api/health` 200 ב-loopback ובשני הדומיינים (guesthub.bios.co.il, stayme.co.il), ‏`/rates` 307
ללא אימות; אין שגיאות בלוג מאז ה-restart (שורות "Server Reference ID" בלוג קדמו לפריסה — טאבים
עם build ישן).

**אימות בדפדפן (magic-link + CDP, קריאה בלבד — טכניקת D173; הסקריפט `verify.mjs` ב-scratchpad
של הסשן).** ‏41/41: כותרת "חדר 1102 · 30/09/2026" + תת-כותרת עם יום בשבוע; רוחב 864px ב-1440
(60%), 1000px ב-1000 (מלא), 390px בטלפון; צ'יפ "ניתן למכירה" + משפט; 9 אריחים; "שמירת מחיר"
disabled כשהשדה שווה לשמור; סטפרים ריקים עם ‏[−] מושבת ו-[+] פעיל; לחיצה על "סגור למכירה"
מהפכת את הצ'יפ **חי** ל"לא זמין למכירה", מדליקה את הראשי ומציגה "שינויים שלא נשמרו: מכירה", והסיכום
בסקשן 4 עוקב אחרי הטיוטה; Esc / רקע / "ביטול" סוגרים וזורקים את הטיוטה (פתיחה מחדש = מצב שמור,
שום דבר לא נכתב); חדר תפוס (1102 · 08/09, הזמנה 1149) — "לא זמין למכירה" עם "קיימת הזמנה
פעילה", אריחים זמין=0/תפוסים=1, שורת "צפייה בלוח"; במגע (‏`hasTouch`, D148) הקשה על תא מחיר
פותחת, ה-`::after` של 44px קיים על כפתור סטפר פעיל (ומוחרג מהמושבת, בכוונה), המתג מהפך חי,
Esc זורק; ‏0 שגיאות קונסול בשלושת הרוחבים. **תרחיש השמירה לא בוצע** באימות הזה: הוא כותב ARI
אמיתי לפרוד ויוצר ג'וב סנכרון ל-Beds24 — `e2e-write-guard` אוסר זאת לאוטומציה בלי אישור.
רונן אישר (22:05Z) זוג כתיבות מתבטל על 1102 · 30/09/2026 אחרי ה-hotfix; התוצאה תתועד בהמשך.

**פגם שנמצא — ארבעת הכרטיסים נחתכים וגוף המגירה לא נגלל (hotfix `.rc-sec { flex: none }`).**
האימות הראשון רץ על viewport בגובה 1500 ולכן לא חשף זאת; צילום ה-390 כן. מדידה חוזרת:
‏1440×900 — גוף client=scroll=740 (אין גלילה), "מצב מסחרי" 328px מתוך 528, "מלאי פיזי"
106/169, "מצב מכירה סופי" 89/141, "סנכרון ערוצים" 140/223; ‏1366×768 — 262/528; ‏390×844 —
311/793; הצ'יפ הסופי לא ניתן ללחיצה באף אחד מהם. השורש: `.rc-body` הופך את `.dw-bd` (מיכל
הגלילה) ל-flex-column, ו-`.card` הוא `overflow: hidden` — ל-flex item עם overflow שאינו visible
`min-height: auto` מתחשב כ-0, ולכן הכרטיסים **מתכווצים** לגובה המיכל במקום לגלוש ולגלול. (המגירה
הישנה השתמשה באותו `flex flex-col` עם אותם `.card` — אותו מנגנון, תוכן קצר יותר.) התיקון: שורה
אחת ב-partial, `.rc-sec { flex: none; }`, מוצמדת בשומר `check:rate-cell-panel`. הכרעת בעלים
(22:05Z): hotfix קדימה, לא rollback.

**תצפיות מחוץ ל-D177 (קיימות לפני השינוי, לא תוקנו):** במובייל עמודת שמות החדרים הדביקה של
גריד התעריפים (`.rg-rlabel`, ~250px) משאירה ~114px לתאי התאריכים ב-390 — הקשה במרכז תא שגולל
ל"מרכז" פוגעת בתווית; שורת הקישור להזמנה מתחת לאריחי המלאי מציגה תאריכי ISO וסטטוס באנגלית
("(confirmed) · 2026-08-27→2026-09-30").

### D177 — תוספת: אימות אחרי ה-hotfix, תרחיש השמירה, והכרעת הבעלים ב-14 הסעיפים (2026-09-07)

**אימות אחרי ה-hotfix** (‏#241, מיזוג `70bffd93`, build `hLV2gjT0aWica8xvT4ofs`). מדידה חוזרת
ב-1440×900 / 1366×768 / 390×844: ארבעת הכרטיסים שלמים (הקופסה = התוכן ‎+2px הבורדר; "מצב מסחרי"
‏530 מול 528), גוף המגירה נגלל (‏1440×900: ‏scroll 1148 מול client 740), והצ'יפ הסופי ניתן
ללחיצה בשלושת הרוחבים.

**תרחיש השמירה** (חדר 1102 · 30/09/2026, ‏su `3037b082…`; זוג כתיבות מתבטל שהבעלים אישר
במפורש — `e2e-write-guard`). ‏14/14: המתג "סגור לעזיבה" מדליק "שינויים שלא נשמרו: כניסה ועזיבה"
ואת הכפתור הראשי; שמירה → טוסט "השינויים נשמרו ליום 30/09/2026"; אחרי הרענון הערך שמור, המחוון
נעלם, הראשי כבוי, והסיכום בסקשן 4 קורא "עזיבה סגורה"; ההחזרה למצב המקורי נשמרה גם היא, ופתיחה
מחדש מציגה את המצב ההתחלתי. אימות DB בקריאה בלבד: שורת `pricing_plan_rates` ‏`51634fef…` חזרה
ל-`closed_to_departure = false` ‏(22:08:05Z; השורה קיימת מיולי — לא נוצרה שורה חדשה), שתי שורות
`audit_logs` מסוג `rate_edit` ‏(22:08:00 true, 22:08:05 false), שני ג'ובי `sync_ari_range`
ל-Beds24 — שניהם `succeeded`, ‏0 שורות `channel_sync_errors`. תופעת לוואי מוצהרת של טכניקת
האימות: `last_sign_in_at` של r@bios מתעדכן בכל ריצת magic-link (הסשן מבוטל ב-`scope=local`).

**הכרעת הבעלים ב-14 הסעיפים (2026-09-07).** תשעה נשארים כפי שנפרסו: 1 (מאגר), 3 (סוג המחיר
ב-hint), 5 (₪ שמור ‎+ "מ׳ הגעה"), 6 (מצבי קריאה בלבד), 7 (גוני הסנכרון), 8 (ירוק מותנה ושבירת
טקסט), 9 (‎#C7CEE8 לשני הראשיים), 11 (הרספונסיב שמעבר למפרט), 13 (עצירה שקטה ב-3650). **חמישה
שונו** (ענף `feat/d177-owner-rulings`, PR נפרד — בלי מיזוג ובלי פריסה עד אישור):

**§2 — משפט ההסבר לא חוזר על הצ'יפ.** מפה חדשה `SELL_REASON_SENTENCE` ב-`rates/types.ts`
("קיימת הזמנה פעילה ביום זה." במקום "לא ניתן למכירה — קיימת הזמנה פעילה"), שמפתחותיה הם בדיוק
הקודים ש-`blockingReasons` יכולה להחזיר — `Exclude<SellReason, "SELLABLE" | "COMMERCIAL_STOP_SELL">`,
כך שקוד חסימה חדש בלי משפט לא מתקמפל (`blockingReasons` קיבלה type predicate לשם כך).
`SELL_REASON_TEXT` נשאר כפי שהוא: הוא ה-tooltip של הגריד, ושם אין צ'יפ שיחזור עליו.

**§4 — "חזרה למחיר הבסיס".** קישור מתחת לשדה המחיר, מוצג **רק** כש-`priceSource === "explicit"`,
שולח `price: null` דרך אותו כלי כתיבה יחיד (הסכמה כבר `nullable()`; ניקוי הוא NULL ולא 0 — 0 היא
סגירה שקטה בפיילוד של Beds24). שדה ריק נשאר לא-תקין, כפי שביקש הבעלים. `SaveKind` קיבל ערך
שלישי, `reset`, כדי ש"שומר…" לא יידלק על כפתור השמירה בזמן איפוס.

**§10 — אישור לפני זריקת טיוטה.** `confirmDiscard` בדפוס `EditReservationPanel`: כל מסלולי
הסגירה (Esc / X / רקע / "ביטול") עוברים ב-`requestClose`, והפוטר מתחלף ל"סגור בלי לשמור" /
"המשך עריכה" ‎+ "יש שינויים שלא נשמרו — לסגור בכל זאת?". מוצג רק כל עוד הטיוטה מלוכלכת
(`confirmDiscard && dirty`), כך שהמצב אינו נתקע אחרי שמירה שנחתה.

**§12 — מוטיבציה מהירה.** `visualVariant="booking"` — הווריאנט הקיים, ולא וריאנט חמישי חדש
ב-`SidePanel`: ‏0.45 שנ׳ עם `cubic-bezier(.32,.72,.24,1)` במקום 1.2 שנ׳. **תופעת לוואי מוצהרת:**
הווריאנט מביא איתו גם את הרקע של חלונות ההזמנה (`rgba(15,23,42,.45)` ‎+ blur 2px במקום
`black/65` ‎+ blur-sm) ואת ההחלקה מ-‎-108% במקום ‎-100%.

**§14 — הסולם הקנוני, כי המדידה לא מצאה חיתוך.** התנאי שקבע הבעלים נבדק על ה-build החי עם
דריסת רוחב בצד הלקוח (`probe-width.mjs`, קריאה בלבד): 12 רוחבים × שני סולמות. **אף אריח לא
נחתך באף רוחב** (`scrollWidth ≤ clientWidth` בכל אריח, בערך ובתווית; אין גלישה אופקית) — ולכן
התנאי לא התקיים והסולם הקנוני (`w-full md:w-[85%] lg:w-[60%]`) נכנס. מה שכן נמדד ומדווח: שישה
אריחי מלאי בשורה דורשים פאנל של **674px**, ולכן בסולם הקנוני השורה נשברת ל-**5+1** בשני פסים —
‏768–792 (‏85%) ו-1024–1123 (‏60%, בלי רצפת ה-700 של המפרט); בסולם שנפרס אלה היו שישה בשורה.
בנוסף, מעל ‎1600px הסולם הקנוני אינו נעצר ב-960 — ‏60% ממשיך לגדול. `.rc-panel` נשאר בלי הצהרת
רוחב ומשמש רק כ-hook ל-`.rc-panel .btn-primary:disabled` (כלל ברזל 9: אין CSS יתום), והשומר
מוודא שמחרוזת הסולם ב-JSX זהה בייט-לבייט לברירת המחדל של `SidePanel` — כדי שהשתיים לא ייפרדו.

**אימות הענף.** `tsc --noEmit` 0 · `eslint` 0 שגיאות (3 אזהרות ישנות ב-`ref/audit`) ·
`check:design` ✔ · `check:responsive` 20/20 · `check:rates-ui` 68 · `check:cell-mark-ladder` 18/18 ·
`check:calendar-ui` ✔ · `check:guard-roots` ✔ · `check:rate-cell-panel` **9/9** (קבוצה 9 חדשה:
לכל קוד חסימה יש משפט, ואף משפט אינו חוזר על הצ'יפ) ‎+ שלושה mutation-tests אדומים (הסרת
`visualVariant`, משפט שחוזר על הצ'יפ, סטייה במחרוזת הסולם). **לא אומת:** תצוגה בדפדפן של חמשת
השינויים — אין build בעץ הפרודקשן (D147); האימות הוויזואלי יתבצע אחרי פריסה, באישור הבעלים.

**הכרעה משלימה (2026-09-07, על #242).** הרקע וההחלקה שווריאנט ה-booking מביא איתו — **מאושרים**.
ה-5+1 — **לתקן**, עם כלל מחייב: **אסור אריח בודד בשורה בשום רוחב בין 320 ל-1440**. המימוש:
`.rc-tiles` עבר מ-`auto-fit` לספירת עמודות מפורשת שהיא תמיד **מחלק** של מספר האריחים — שישה
עושים 6 / 3×2 / 2×3, שלושה עושים תמיד 3 — לפי **רוחב הכרטיס** (‏`container-type: inline-size`
על `.rc-sec .card-bd`, אותו דפוס כמו `.gu-body` ב-group-update), כי המגירה היא שבר מהחלון ולכן
רוחב החלון אינו אומר דבר על רוחב הכרטיס. הספים הם מה שהפריסות באמת דורשות — ‏578px = 6×88+5×10,
‏284px = 3×88+2×10 — כך שהטלפון שומר בדיוק את ה-3×2 (ו-2×3 ב-320) שהיה לו. **הפגם היה רחב
משני הפסים שסומנו:** כל כרטיס ברוחב 480–577px נתן 5+1, כלומר גם חלון ‎~600px ולא רק
768–792 ו-1024–1123; והרשת בת שלושת אריחי הסנכרון נתנה 2+1 ב-320 ו-360. (הפסים המדויקים הם
768–792 ו-1024–1123 ולא ‎…–793 / ‎…–1124 כפי שדיווחתי תחילה: ב-793 וב-1124 רוחב תוכן הכרטיס הוא
בדיוק 578px והשישה נכנסים לשורה אחת.) השומר אוכף שכל ספירת
עמודות היא מחלק של מספר האריחים, שאין `auto-fit`, שקיים container query, ושכל שורה אכן מחזיקה
שישה/שלושה אריחים. ‏`3×2` לא חתך אף תווית באף רוחב, ולכן חלופת ה-2×3 של הבעלים לא נדרשה.

---

## D178 — מגירת "שליחת מייל לאורח" לפי העיצוב המאושר: תצוגה בלבד, ארבע הכרעות בעלים (2026-09-07)

**מה השתנה.** ‏`MessageComposer` (D53) נבנה מחדש לפי העיצוב המאושר
`שליחת מייל לאורח.dc.html` (המפרט הכתוב + שלושה צילומי מסך; **הקובץ עצמו לא נמצא בדיסק** —
בדיוק כמו ב-D175 וב-D177 — ולכן שמו שורד רק כמחרוזת ציטוט בסמני `ds-allow:`). הבאנר הוורוד
שחצה את כל רוחב החלון, כרטיס הנמען הריק, שני הטאבים הלבנים, רשימת המשתנים כטקסט מת והתצוגה
המקדימה חסרת המעטפת — הוחלפו במעטפת כרטיסים תקנית: אזהרה **בתוך** כרטיס הנמען, אווטאר +
מקור הזמנה + צ'יפ סטטוס כתובת, בורר segmented, צ'יפי משתנים לחיצים שמוסיפים במיקום הסמן,
וכרטיס מייל אמיתי בתצוגה המקדימה.

**זה שינוי תצוגה בלבד.** ‏`sendBookingEmailAction` / `sendBookingWhatsAppAction` /
`sendEmailMessage` — לא נגעו. הדבר היחיד ש-`message-actions.ts` קיבל הוא חמישה שדות תצוגה
ב-`ComposerContext` (`reservationNumber`, `roomNumbers`, `checkIn`, `checkOut`, `sourceLabel`)
שכבר היו מחושבים ב-`buildContext` — בלי שאילתה חדשה ובלי round-trip נוסף.

### ארבע הכרעות הבעלים (2026-09-07)

1. **המגירה נשארת overlay** בתוך מגירת ההזמנה, ולא SidePanel שני. כל הכרום החדש מיושם,
   הרוחב נורש ממגירת ההזמנה (60%, 900–1200) ו-`.sm-panel` לא מצהיר `width` כלל. כך D53
   נשמר: ההזמנה טעונה מתחת וסגירה משחזרת את מצב הגלילה במדויק. (המפרט ביקש רוחב עצמאי
   60% / 700–960 — לא מיושם, לפי ההכרעה.)
2. **שני הערוצים לובשים את העיצוב.** WhatsApp מקבל את אותה מעטפת: צ'יפ טלפון במקום מייל,
   בלי שדה נושא, ותצוגה מקדימה בלי שורת "נושא". קומפוננטה אחת, אין שני עיצובים לאותו חלון.
3. **התצוגה המקדימה נאמנה לחוט.** משתנה מוכר שערכו ריק מוצג **ריק** — בדיוק כפי שיישלח.
   ה-"—" שהמפרט ביקש **לא מיושם**: הוא היה מראה למפעיל משהו שלא יוצא במייל. משתנה
   לא-מוכר ממשיך לחסום את השליחה ולהיקרא בשמו (D172 + התוספת מ-2026-09-05).
   ה-"—" היחיד שנשאר הוא שורת `אל: — (חסרה כתובת)` שבמפרט.
4. **כל 16 המשתנים** של `CANONICAL_VARIABLES` מוצעים כצ'יפים, כולל "סטטוס" ו"מקור"
   שרשימת ה-14 שבמפרט השמיטה — המפרט עצמו כתב "לפי המשתנים הקיימים במערכת", והסתרת שניים
   הייתה נסיגה ביכולת.

### היכן המערכת גברה על המוקאפ (תקדים D175/D177)

- **כרום הכותרת:** ‏`.dw-icon` (40px) ו-`.dw-close` (36px) מ-`design-system.css` — הפרטיל
  **לא מצהיר אותם מחדש**, הוא צורך אותם. המוקאפ ביקש 42/38 (GUIDELINES §7 גובר).
- **טוסט:** ה-`sonner` המערכתי, 2.8 שנ׳ (§9), לא 2.2 ולא טוסט משלו.
- **קווים:** ‏`var(--line)` ולא `#EEF0F5` (§12.2). **צל כרטיס:** ‏`var(--shadow-card)` (§6).
- אייקון `public` נרשם כ-`"globe-filled"` ולא דרס את `globe` הקיים (`language`) — כדי לא
  להחליף בשקט את הגלגל בכל אתרי הקריאה האחרים.

### CSS ואין-יתומים (כלל ברזל 9)

הפרטיל החדש `src/app/styles/send-message-panel.css` (prefix `sm-`, לא-משוכב כמו
`rate-cell-panel.css`, כי `p-0` של SidePanel הוא utility). **ה-`.bk-cmp*` לא נמחק כולו:**
בדיקת grep לפני המחיקה גילתה שהמעטפת (`.bk-cmp`, `-h`, `-back`, `-icon`, `-body`) משרתת גם
את `CancelReservationDialog` וגם את `BookingComReports` — נמחקו רק 115 השורות שהיו של
הקומפוזר בלבד (`-f`, `-recipient`, `-tabs`, `-tab`, `-textarea`, `-vars*`, `-preview*`,
`-alert*`, `-ok`). 21 סמני `ds-allow: שליחת מייל לאורח.dc.html` מכסים את מה שהמוקאפ קובע
מחוץ לסקאלות הסגורות: צבעי הצ'יפים והאזהרות (`#E7F6EC/#15803D`, `#FDECEF/#BE123C`,
`#FDF1F4/#F5C9D4/#9F1239`), רקע השדה `#EFF2FB`, הראשי הנעול `#C7CEE8`, גבהי 46/48/52,
רדיוס 13 וטבעת הבורר.

### לוגיקה טהורה ושומר

`src/lib/messaging/composer-draft.ts` (בלי imports, כמו `cell-draft.ts`) מחזיק את
`insertToken`, את `manualSendGate` + `sendBlockMessage` — **שער אחד** שמניע גם את נעילת
הכפתור וגם את שורת הסיבה בפוטר, כך שכפתור נעול תמיד אומר למה — ואת `guestInitials` /
`formatStayRange`. סדר הסיבות במתכוון: הגורם ה**עליון** הוא זה שמוצג, כך ש"חסרה כתובת
אימייל" לא מתחבא מאחורי "יש למלא נושא".

`scripts/check-send-message-panel.mjs` (‏`check:send-message-panel`) מקמפל את המודול הטהור
ו**מריץ** אותו, ואוכף סטטית את ארבע ההכרעות, את שתי החריגות שבהן המערכת גברה, את היעדר
היתומים ואת החיווט. אומת שאינו ריק: ביטול בדיקת הנמען בשער → 4 כשלים; סינון צ'יפ אחד
מהרשימה → 2 כשלים.

### "עדכון פרטי האורח"

הקישור מופיע **רק** כשאין כתובת תקינה. הוא סוגר את ה-overlay וממקד את שדה האימייל בכרטיס
"פרטי אורח" של ההזמנה. הטיוטה (מצב/תבנית/נושא/תוכן) הורמה ל-`EditReservationPanel` ומוחזקת
**לכל ערוץ בנפרד**, ולכן היא שורדת את הסגירה. אחרי ההקלדה הטופס `dirty`, ולכן
`guardedToolbarAction` יחזיק את המגירה סגורה עד **שמירה** — וזה נכון: השליחה קוראת את
ההזמנה השמורה הקנונית, לא את הטופס הפתוח.

### החזרה (2026-09-08)

הוחזר ב-PR זה אחרי revert (‏#245). סיבת החיתוך: D181. אומת ב-CDP 390×844 לפני מיזוג.
פתוח: איפוס גוף במעבר ל"הודעה חדשה" — הכרעת בעלים.

### איפוס הגוף והנושא במעבר ל"כתיבת הודעה חדשה" — הוכרע (2026-09-10)

**הכרעת הבעלים:** המעבר ל"כתיבת הודעה חדשה" **מרוקן את גוף ההודעה ואת הנושא לגמרי** —
שני השדות ריקים, בלי טקסט התבנית ובלי `{{placeholders}}`, ולכן גם התצוגה המקדימה הנגזרת
מהם מתרוקנת. ריק זה ריק. המעבר חזרה לתבנית **ממלא את שניהם מחדש**.

> ההכרעה ניתנה בשני שלבים באותו יום: תחילה הגוף בלבד, ואז — אחרי שהצילום הראה שהנושא
> נשאר עם `{{placeholders}}` שלו — הורחבה במפורש גם לנושא (10/09/2026).

**מה היה.** `patch({ mode: "custom" })` (‏`BookingActions.tsx:371`) פרש את כל ה-draft
(‏`:148`), ולכן כל שדה חוץ מ-`mode` שרד. ההתנהגות קיימת מ-`ab6609e` ואינה רגרסיה של D178,
אבל היא הייתה **מטעה ולא רק דביקה**: התצוגה המקדימה לצד העורך מרנדרת את אותו גוף עם הערכים
פתורים (`:202-203`), כך שהמפעיל ראה טוקנים גולמיים בעורך וערכים אמיתיים בתצוגה — לאותה
הודעה, באותו רגע.

**החצי השני אינו קוסמטי.** החזרה לתבנית לא מילאה מחדש גם קודם, מסיבה נפרדת: ה-`<select>`
שומר את `value={templateId}` (‏`:381`), ולכן בחירה חוזרת של **אותה** תבנית אינה יורה אירוע
`change` ו-`applyTemplate` לא היה רץ שוב לעולם. בלי הענף הזה המסלול תבנית → חדשה → תבנית
היה חד-כיווני והגוף לא היה חוזר כלל.

**איפה זה יושב.** המעבר עבר ל-`applyMode` ב-`src/lib/messaging/composer-draft.ts` — המודול
הטהור חסר-הייבוא ש-`check:send-message-panel` כבר מקמפל וקורא לו — ולכן ההתנהגות **מוכחת
בהרצה**. הקומפוזר מחזיק שתי עטיפות בלבד: `applyTemplate` (בחירה ב-`<select>`) ו-`switchMode`
(שני כפתורי המקטע). `templateId` **נשמר** במעבר לכתיבה חופשית — זה מה שנותן למעבר חזרה
ממה למלא; חוזה השליחה כבר שלח `templateId: null` במצב `custom` (‏`:223-224`) ולא נגענו בו.

**הנושא מאופס גם הוא.** המימוש הראשון ניקה את הגוף בלבד, כפי שההכרעה הראשונה נסחה,
והצילום חשף ששדה הנושא נשאר עם `{{placeholders}}` של התבנית. ההכרעה הורחבה, והאיפוס
יושב באותה שורה: `applyMode` מחזיר `{ ...draft, mode, body: "", subject: "" }`
(`composer-draft.ts:60`). ב-WhatsApp אין שדה נושא כלל, ולכן שם האיפוס בלתי-נראה
והמילוי-מחדש אינו מחזיר נושא — הענף מותנה ב-`isEmail`, כפי שהיה.

**תופעת לוואי מכוונת של הסדר בשער.** במייל, אחרי המעבר, `manualSendGate` נועל כעת על
`subject_empty` ולא על `body_empty` — הנושא נבדק לפני הגוף, כי הסיבה שבמעלה הזרם היא
זו שנאמרת. הכותרת התחתונה אומרת "יש למלא נושא להודעה". זה השער הקיים, לא כלל חדש.

**שומר.** `check:send-message-panel` קיבל קבוצה עשירית: 16 טענות runtime על `applyMode`
המקומפל + 4 טענות חיווט — כולל שהנושא מתרוקן, שאין בו `{{`, שהמעבר חזרה מחזיר את **שניהם**,
ושהאיפוס שייך למעבר **לתוך** `custom` בלבד ולא לכל מעבר. ארבע מוטציות אדוורסריות (ביטול האיפוס, קוד-מת בראש הפונקציה,
חזרה ל-`patch({ mode })` בכפתור, ונטרול ענף המילוי-מחדש) הפילו אותו ל-exit 1 ושוחזרו לירוק.

**אימות ויזואלי (2026-09-10, ‏390×844).** על fixture סטטי שטוען את ארבעת קבצי ה-CSS
**האמיתיים** מ-`src/app/styles` (‏design-system, base, send-message-panel, responsive —
אומתו זהים ב-sha256 למה שבריפו) ומייבא את `composer-draft.ts` המקומפל, כך שההיגיון בצילום
הוא הקוד שנשלח. נמדד: אין גלישה אופקית (390/390), אף כרטיס אינו נחתך בשלושת המצבים
(`scrollHeight ≈ clientHeight`), ו-`.sm-body` גולל (‏604/1184) — כלומר תיקון D181 מחזיק.
המעבר: `body=""` **וגם** `subject=""`, שני האלמנטים ב-DOM ריקים ומציגים placeholder,
אפס `{{` בשניהם, כפתור השליחה **ננעל** והכותרת התחתונה אמרה "יש למלא נושא להודעה" —
...
ה-automatic minimum size של פריט flex (`min-height: auto`) מתפוגג ל-**0** כשה-overflow שלו
אינו `visible`. לכן `.card` שיושב בתוך מיכל `flex-direction: column` חסום-גובה **מתכווץ מתחת
לגובה התוכן** במקום לגלוש, וה-`overflow: hidden` שלו עצמו חותך את גופו — והמיכל הגולל נשאר
בלי מה לגלול. כל מיכל כזה חייב להעניק לכרטיסים שבו `flex: none` (או `flex-shrink: 0`).

**תקדים D177 נכשל כי נכתב על `.rc-sec` בלבד.** הבאג נמדד בפרודקשן שלוש פעמים:
`.gc-col` בעורך התקשורת (תוקן ב-`.gc-col > * { flex-shrink: 0 }`, communications.css:359),
D177 במגירת מצב-מכירה 2026-09-06 (‏328 מתוך 528 פיקסלים הוצגו; תוקן ב-`.rc-sec { flex: none }`),
ו-D178 בפאנל שליחת ההודעה 2026-09-07 (‏390×844: הכרטיסים רונדרו 126/194, 248/384 ו-200/309,
ו-`.sm-body` לא היה גליל כלל; רוורט ב-PR #245). D177 אכן הצפין את הלקח — ב-
`check-rate-cell-panel.mjs:221` — אבל ככתיבה על **פאנל אחד** ולא על **הפרימיטיב**, ולכן לא
יכול היה לראות את הפאנל הבא. `check:flex-card-shrink` הוא ההכללה: הוא מאחד את ההצהרות של
המחלקות שיושבות **יחד על אלמנט אחד** ב-JSX (כולל קפיצה אחת דרך prop של className, כי
`SidePanel` מרכיב `dw-bd` של עצמו עם `rc-body` של הקורא), מוצא את הכרטיסים בהכלה מבנית
אמיתית (כולל דרך קומפוננטת-עזר מקומית, כמו `<Section>` של D177), ומקבל שלושה מסלולי מילוט:
כלל על הכרטיס עצמו, כלל צאצא מהמיכל, או כלל ילדים אוניברסלי כמו `.gc-col > *`.

**מה נמצא על main (‏a65cb1d):** 15 מיכלים גוללים; `.rc-body` ו-`.gc-col` **עוברים** (התיקונים
הקיימים), ו-**9 ממצאים על `.bk-cmp-body`** — ‏4 ב-`BookingComReports.tsx` ו-5 ב-
`CancelReservationDialog.tsx`. שני ה-overlays האלה חולקים את המעטפת שהקומפוזר הישן הותיר.
**ה-CSS לא תוקן ב-PR הזה בהוראת הבעלים** — תיקון ה-CSS הוא PR נפרד. עד אז השומר אדום על main.

**גבול ידוע (נמדד).** הכלל מנוסח על מיכל **גולל**, ולכן הסרת `overflow-y` מ-`.sm-body`
מוציאה אותו מטווח השומר: השומר מוריק, והפאנל אינו נחתך יותר — אבל 318px מתוכנו הופכים
בלתי-נגישים, כי אין גלילה בשום מקום. זהו **שלילי-שקר**, לא חיובי-שקר, והוא מעקף אמיתי.
מיכל flex-column חסום-גובה שאינו גולל דורש ניתוח אבות ולכן נשאר מחוץ ל-PR הזה.

**פתוח.** D178 — מעבר ל"הודעה חדשה" לא מאפס גוף תבנית: הטיוטה נשמרת ולכן ה-`{{placeholders}}`
של התבנית נשארים גלויים (`BookingActions.tsx:371` + `:148`). **התנהגות קיימת מ-`ab6609e`**
(‏`ab6609e:BookingActions.tsx:300` — `setMode("custom")` בלי `setBody("")`) ולכן אינה רגרסיה
של D178; חוזה השליחה תקין בשני המקרים. **ממתין להכרעה** אם זו שמירת-עבודה מכוונת או פגם.

## D181 (המשך) — `.bk-cmp-body` תוקן; שלושת התקדימים מיושרים לאותו כלל (2026-09-08)

> הכלל עצמו נוסח ב-D181 (‏PR #246, `check:flex-card-shrink`) ואינו חוזר כאן. אם #246 טרם מוזג,
> הפריט הזה עומד בפני עצמו: `.card` הוא `overflow: hidden`, ולכן ה-automatic minimum size שלו
> מתפוגג ל-0 בתוך flex-column חסום-גובה והוא מתכווץ וחותך את גופו במקום לתת למיכל לגלול.

**`.bk-cmp-body` תוקן** ב-`.bk-cmp-body > .card { flex: none; }` (‏booking-window.css:1253).
זו המעטפת שהקומפוזר הישן הותיר לשני ה-overlays שנשארו — `CancelReservationDialog` ו-
`BookingComReports` — ולכן שניהם היו שבורים בפרודקשן, לא רק תיאורטית.

**נמדד ב-Chrome, 390×844, לפני/אחרי:** בביטול ההזמנה ארבעת הכרטיסים רונדרו 152/233, 82/124,
131/201 ו-171/243 (‏265px נחתכו) ו-`.bk-cmp-body` לא היה גליל; בדיווח ל-Booking.com 115/138,
177/215, 170/206 ו-74/80 (‏103px נחתכו), גם הוא לא גליל. אחרי התיקון כל שמונת הכרטיסים
מרנדרים את מלוא תוכנם ושני הפאנלים גוללים (‏904 ו-759 מול 631 זמינים). `check:flex-card-shrink`
עבר מ-9 ממצאים ל-**0**.

**שלושת התקדימים מיושרים כעת לאותו כלל:** `.gc-col > * { flex-shrink: 0 }` (עורך התקשורת),
`.rc-sec { flex: none }` (‏D177), ו-`.bk-cmp-body > .card { flex: none }` (כאן). אין יותר
flex-column גולל בקוד הבסיס שמכווץ כרטיסים.

## D182 — עדכון קבוצתי: טווח התאריכים הוא לילות, "עד תאריך" הוא צ'ק-אאוט (בלעדי) (2026-09-10)

**הבאג (דווח 2026-09-10).** בפאנל "עדכון קבוצתי" בחירה של `מתאריך=16/09/2026`,
`עד תאריך=17/09/2026` הציגה "2 לילות" והכתיבה הקבוצתית עדכנה את **שני** הלילות — גם 16/09
וגם 17/09. מלון מוכר **לילות**: 16→17 הוא לילה אחד, הלילה של ה-16. כך זה ב-Booking.com,
בכל הערוצים ובאתר הציבורי, וכך חייב להתנהג גם העדכון הקבוצתי.

**הכרעות הבעלים (אושרו 10/09/2026, מיושמות ככתבן):**
1. סמנטיקת הטווח: `from` = הלילה הראשון, `to` = צ'ק-אאוט (בלעדי). ‎20→21 = לילה **אחד**
   (ה-20). `from == to` **חסום** (אפס לילות) בכל שכבה.
2. הפריסטים 7/14/30 הם **לילות**: `to = addDays(from, n)`. התוויות: "7 לילות" / "14 לילות" /
   "30 לילות".
3. תוויות הבורר נשארות בדיוק "מתאריך" / "עד תאריך". רק ספירת הלילות משתנה.
4. שורות היסטוריות ב-`bulk_rate_update_logs`: **ללא שינוי דאטה**. מתועד ב-DECISIONS בלבד.
5. נשלח לפני החזרת D178.

**מה השתנה בקוד.** הפאנל עבר ל-`mode="nights"` — אותו בורר קנוני, בסמנטיקת שהות:
`pickRange` בלי `allowSameDay` **מעגן מחדש** במקום לסגור טווח של אפס לילות
(`date-range.ts:31-32`), ומונה הלילות בבורר יורד ל-`nightsBetween(from, to)` בלי ה-`+1`
(`DateRangeField.tsx:80-81`). ההרחבה בלקוח (`GroupUpdatePanel.tsx`) ובשרת
(`rates/actions.ts:134`) הפכו שתיהן לחצי-פתוחות `eachDay(dateFrom, dateTo)`, והסכימה
דורשת `dateTo > dateFrom` עם הודעה שאומרת מה הכלל
(`validation/rates.ts`). הזריעה מחלון הגריד מוסיפה יום — הגריד מוסר `toInclusive` (לילה),
והפאנל מחזיק צ'ק-אאוט — כך שחלון של 30 יום בטבלה נשאר 30 לילות.

**`DateRangeField.tsx` ו-`date-range.ts` לא נגעו.** התוויות "מתאריך"/"עד תאריך" מקודדות
קשיח מחוץ לכל הסתעפות `mode` (‏`DateRangeField.tsx:176,186`), ולכן הכרעה 3 מתקיימת בלי
שום שינוי בקומפוננטה המשותפת ובלי prop `labels`. הדבר היחיד שהפאנל נאלץ להעביר הוא
`hint` — ברירת המחדל של מצב nights מפנה לכפתור "שמור שינויים" של טופס, ולפאנל הזה יש
כפתור עדכון משלו; ה-prop הזה כבר היה קיים.

**מסנן ההזמנות נשאר `mode="days"`.** הוא חלון סינון ולא שהות: שני הקצוות כוללים
(`reservations/data.ts` — `>= from AND <= to`) ויום בודד הוא בחירה חוקית. השומר נועל את
זה מפורשות כדי שהצרכן שלא נגענו בו לא ייסחף.

**הסמנטיקה הכוללת מעולם לא הייתה החלטת בעלים.** אין ולא היה ערך ב-DECISIONS.md על
"סוף כולל" / `mode="days"` / `allowSameDay`. הכלל קודד ב-`2fb31f8` (לולאת השרת
`[from, to]`) וב-`0a55640` (הפאנל, `addDays(dateTo, 1)` ופריסט `n - 1`) ב-04/07/2026,
במעקב אחר התנהגות מרומזת של מוקאפ ה-HTML של הבעלים (`while(d<=to…)`), והוצהר במפורש רק
בגוף הקומיט של `964a882` (‏PR #79, 13/07/2026) — לא בקובץ ההחלטות. ארבעת האזכורים
"(D93)" ב-`docs/audit/*` הם **הפניות תלויות**: הם נכתבו ב-18/07/2026, כש-DECISIONS.md
הסתיים ב-D76 ו-D93 עוד לא היה קיים; D93 בפועל (24/07/2026) הוא ביטולי OTA. ההערה
"§7b" ב-`actions.ts` מפנה לדוח שאינו בריפו ואי אפשר לאמת ממנו הכרעה.

**היסטוריה ב-DB — ללא backfill.** `bulk_rate_update_logs.date_to` שנכתב **לפני** פריסת
D182 = הלילה האחרון, כולל; **אחרי** = צ'ק-אאוט, בלעדי. אין מיגרציה ואין כתיבת דאטה.
נמדד בפרודקשן לפני השינוי: מתוך 93 שורות, ב-88 `MAX(items.date) = date_to` (הפער
ב-5 הנותרות נובע מסינון ימי-שבוע), ו-3 שורות הן `date_from = date_to` — עדכונים של יום
בודד, שתחת הכלל החדש כבר לא ניתן ליצור. הטבלה אינה נקראת לתצוגה בשום מקום במוצר: הקוראים
היחידים הם `COUNT(*)` (חוסם מחיקת חדר ושני סקריפטי בדיקה), ולכן אין מסך שמציג טווח ישן
בסמנטיקה החדשה.

**האופק — צ'ק-אאוט מותר יום אחד אחרי (הכרעת בעלים, אותו PR).** בדיקת האופק בשרת ותקרת
הבורר נוסחו שתיהן על `dateTo` כעל **לילה** (תקרה = `latest`). משעברנו לצ'ק-אאוט, תקרה כזו
הייתה מוציאה מהישג יד בדיוק את הלילה האחרון: כדי לכתוב את הלילה `latest` צריך צ'ק-אאוט
`latest + 1`. לכן `rates/actions.ts` בודק כעת `input.dateTo > addDays(latest, 1)`, והפאנל
מחזיק **שתי** תקרות — `clampDate` ל-`dateFrom` (לילה, עד `maxDate`) ו-`clampCheckOut`
ל-`dateTo` (צ'ק-אאוט, עד `maxCheckOut = addDays(maxDate, 1)`), שגם עוברת ל-`max` של הבורר
ולפריסטים. ל-`dateFrom` אין תקרה משלו: הסכימה אוכפת `dateTo > dateFrom`, ולכן
`dateFrom <= latest` נובע. נמדד על הקוד המשוגר: `from=latest, to=latest+1` מתקבל ומתרחב
בדיוק ל-`[latest]`; `to=latest+2` נדחה.

**הקבוע המשותף לא נגע.** `RATES_HORIZON_YEARS`, `ratesWritableWindow`,
`isRateDateWritable` ו-`clampRatesFrom` ב-`lib/dates.ts` נשארו כפי שהם, וכך גם חלון הגריד
(`rates/page.tsx`), ניווט הגריד, הכתיבה לתא בודד (`upsertRateCellAction`, שבה `input.date`
הוא לילה ותקרתו `latest`), `rate-plans/actions.ts` ו-`CellDetailPanel`. ה-`+1` נוסף אך ורק
בשני אתרי התקרה של העדכון הקבוצתי.

**שומר.** `scripts/check-datepicker.mjs` עודכן — לא עוקף. הוא אוכף כעת: הפאנל מרנדר
`mode="nights"` (נבדק על **האלמנט**, לא על הקובץ, כדי שהערה לא תוכל לספק את התנאי);
השרת מרחיב `eachDay(input.dateFrom, input.dateTo)` ואין בו `addDays(input.dateTo, …)`;
הסכימה **מורצת** ודוחה `dateTo === dateFrom` (בדיקה התנהגותית, לא regex); מסנן ההזמנות
נשאר `mode="days"`; ושתי תקרות האופק מאפשרות צ'ק-אאוט של `latest + 1`, כך שהלילה האחרון
נשאר בהישג יד. כל אחת מחמש האסרשנים הופרכה בנפרד (נטרול הפרדיקט → יציאה 1 → שחזור).

## D179 — `check:manual-send-render` מריץ את שער השליחה במקום לקרוא אותו (2026-09-10)

> **מספר חדש בקובץ.** ‏D179 שימש כשם-עבודה לפער הזה מאז 2026-09-07, אבל רשומה לא נכתבה
> אז ולא קיימת כאן. הפריט הזה הוא הרשומה הראשונה תחת המספר. (הכותרת "תחת מספרים קיימים"
> בתדריך לא הייתה מדויקת — D174, D176, D179 ו-D180 כולם חסרים מהקובץ.)

**הפער, כפי שנמדד ב-2026-09-07** באימות שלפני מיזוג #244. השומר הצהיר על שרשרת שלמה —
"משתנה שלא ניתן לפתור אינו מגיע לאורח" — אבל אימת את החוליה האחרונה, `manualSendGate`,
בשתי טענות grep על טקסט המקור. שלוש מוטציות נבדקו אז: הסרת שתי שורות הנעילה נתפסה,
ניתוק הקומפוזר מהשער נתפס, ו-**קוד מת בראש ה-IIFE — כל תו במקומו, הסמנטיקה מתה — עבר
בירוק**. grep אינו יודע להבחין בין ענף חי לענף שאיש אינו מגיע אליו.

**מה נעשה.** ‏`Part C` מקמפל את `composer-draft.ts` יחד עם `render-manual.ts` (שניהם מודולים
טהורים; ל-`composer-draft.ts` אין ולו ייבוא אחד) ומזין לשער את הוורדיקטים **האמיתיים** של
הרנדרר, בשכפול מדויק של החיווט ב-`BookingActions.tsx:197-213`. שש סצנות: תבנית נקייה אינה
ננעלת; טוקן מנוקד לא-מוכר בגוף נועל בפועל את הכפתור תחת `body_blocked` עם המשפט העברי;
אותו דבר בנושא; בשני שדות חסומים הנושא נאמר ראשון (הסיבה שבמעלה הזרם גוברת); גוף שכולו
טוקן `{{snake_case}}` לא-מוכר מתרנדר לריק ונחסם כ-`body_empty`; ואותו נושא חסום נועל מייל
ואינו נועל WhatsApp, כי ל-WhatsApp אין שדה נושא ולכן `subjectBlocked` שם תמיד false.

שתי טענות ה-grep על השער (‏`if (input.subjectBlocked) …` / `bodyBlocked`) **הוסרו** — ‏Part C
מוכיח אותן בהרצה. שלוש הטענות על הקומפוזר נשארו טקסטואליות: ‏`BookingActions.tsx` הוא
קומפוננטת React ואינו ניתן להרצה בשומר בלי סביבת Next.

**ההפרכה.** אותה מוטציה בדיוק (`})() && null` — ה-IIFE מעריך כל ענף, התוצאה נזרקת, הכול
עובר טיפוסים) הורצה מול שתי גרסאות השומר: מול הגרסה **שעל main** — `PASS exit 0`;
מול Part C — `exit 1` עם שש טענות אדומות. זה הפער, ממופה וסגור. עוד שלוש מוטציות
(נטרול נעילת הגוף, החלפת סדר שתי הנעילות, ו-`canSend` שמפסיק לעקוב אחרי `block`) הפילו
אותו כל אחת בנפרד.

**הערת דיוק לרשומה של 07/09.** נמדד היום ש-`check:send-message-panel` — שומר אחר — כן תופס
את מוטציית קוד-המת, כי הוא מריץ את `manualSendGate` מאז D178. הפער אינו "אף אחד לא תפס",
אלא **השומר שבבעלותו הטענה הוכיח אותה בקריאה והישען על שכן**. גם זה פער: מי שימחק את השכן
או ישנה את היקפו יוריד כיסוי בלי לדעת.

## D183 — ה-`minStay` של Beds24 נושא את `min_stay_through`; ה-read-back משווה אותו (2026-09-11)

**הבאג.** ללוח השנה של Beds24 יש שדה מינימום-לילות יומי **אחד**, וכל 15 החדרים הממופים
מוגדרים `restrictionStrategy: "stayThrough"` — כלומר הערך מחייב כל לילה בשהייה. בונה ה-payload
הזין אותו דווקא מגרסת ההגעה (`minStayArrival`), בעוד `minStayThrough` — העמודה ששדה
"מינימום לילות" ברשת התעריפים כותב אליה — חושב, נישא על שורת הביניים, ולא נקרא על ידי איש.
ומכיוון ש-`min_stay_arrival` הוא NULL בכל הטננט מאז **31/07**, ברירת המחדל של התוכנית (1) ניצחה:
השדה לא רק לא נשלח — **כל דחיפה פרסמה `minStay: 1`**, ומאחר ש-`POST /inventory/rooms/calendar`
הוא עדכון חלקי בלי פעולת מחיקה, כל drain **אישרר מחדש** את ה-1 במעלה הזרם ודרס כל הגבלה
שהוגדרה באקסטרנט.

**שרשרת הראיות (הזמנה 1164).** ב-06/09 18:14:23Z כתב הבעלים `minStayThrough: 2` על 11–12/09
ל-16 יחידות (‏`bulk_rate_update_logs` a2355e08, בלי מפתח מחיר כלל). ‏0.47 שניות אחר כך נוצר
dirty range מסוג `restrictions` והסתיים `synced`; יצאה קריאת HTTP אמיתית (‏`channel_evidence_ledger`
‏26092511, ‏`outcome=success`, ומד הקרדיט של Beds24 ירד — מה שלא ניתן לזייף). ב-11/09 11:13:15Z,
ארבעה ימים ו-17 שעות אחר כך, מכרה Booking.com לילה בודד ב-11/09 בחדר 1245. קריאה חיה מ-Beds24
באותו יום: `minStay: 1` על אותו תאריך, בכל 15 החדרים, בכל 27 הטווחים. המחירים דווקא הגיעו
לשם מדויקים עד גבולות הטווחים — ההובלה הייתה בריאה; רק הערך הזה היה שגוי.

**למה זה שרד שישה שבועות.** ‏`beds24-ari-readback` דילג על ההגבלות **בכוונה**: הוא לא ביקש את
השדה מ-Beds24, לא פירסר אותו, ולא השווה אותו. ‏344 מחזורים רצופים דיווחו `driftCells=0` על
210 תאים — ‏335 מהם לפני המכירה. ובמקביל GuestHub **כן** דיווח על עצמו: שלוש דקות אחרי הייבוא
נכתבה השורה היחידה מסוג `OTA_STAY_RESTRICTION_VIOLATION` בכל הטבלה. איש לא ראה אותה.

**ארבע ההכרעות (בעלים, 11/09/2026).**
1. שדה ה-`minStay` היחיד מוזן מ-`min_stay_through` בלבד. ‏`min_stay_arrival` הוא פנימי (אתר,
   הזמנות ידניות) ואינו נשלח ל-Beds24 — החישוב מבוסס-ההגעה **נמחק** משכבת הערוץ, לא סומן
   כמת: הבאג עצמו היה שדה שמחושב ואיש לא קורא, ולהשאיר כזה במראה הפוכה היה לשחזר אותו.
2. ה-read-back משווה `minStay` ו-`maxStay`. ההשוואה אוכפת רק תאים שבהם **אנחנו** הצהרנו ערך —
   אותו כלל שהמחיר תמיד השתמש בו — וזה מה שמנטרל את החשש שהדילוג המקורי נכתב סביבו
   (‏apiV2.yaml: ערך שלא הוגדר בלוח השנה מוחזר מרמת החדר, ולכן תא שלא הצהרנו עליו אינו בר-ייחוס).
3. הפרות הגבלה של OTA יוצגו בפאנל "הזמנות תקועות מערוץ" עם סימון בולט עד סימון כפתור —
   **PR נפרד, אחרי זה**.
4. אין דחיפה מחדש גורפת של הגבלות. הערכים יזרמו בכתיבה הבאה.

**הוכחה.** הבונה על ההשלכה החיה, חדר 707494 / 11–12/09: לפני `minStay: 1`, אחרי `minStay: 2`.
‏Dry-run של ה-read-back מול Beds24 (GET בלבד): ‏210 תאים הושוו, **30 drift, כולם `minStay`,
כולם 11–12/09, ‏`we=2 beds24=1`, ואפס בכל השאר** — כולל אפס drift ב-`maxStay`, שמאשר ש-maxStay
מעולם לא סבל מהבאג (אין לו פיצול through/arrival). ‏30 ולא 32: העדכון הקבוצתי נגע ב-16 יחידות,
אחת מהן ("חניה זמנית") אינה ממופה ל-Beds24, ולכן 15 חדרים מפורסמים × 2 תאריכים.
שומר: `check:beds24-minstay-source` — מריץ את הבונה האמיתי על through=3/arrival=5 ודורש 3,
ומריץ את ה-diff האמיתי. ‏`check:beds24-ari-drain` תוקן וחוזק: הוא זרע `min_stay_arrival` ואימת
אותו על החוט — עכשיו הוא זורע את שתי העמודות בערכים שונים, כך שהטענה עוברת רק מהעמודה הנכונה.

**שני פערים שנשארים פתוחים להכרעת הבעלים (לא נסגרו כאן).**
- **ה-payload היוצא אינו נלכד בשום מקום.** כל 43,140 שורות `sync_ari_range` נושאות `payload = '{}'`,
  ו-`request_payload` נכתב רק בכישלון. לכן גוף החוט של דחיפה מוצלחת אינו ניתן לשחזור מהמסד
  לאף תאריך ולאף חדר — הוא רק משוחזר מקוד. זו החוליה החלשה היחידה בשרשרת הראיות של D183.
- **`bulk_rate_update_items` מתעד רק מחיר.** יש בו `old_price`/`new_price` בלבד ואין עמודת הגבלה,
  ולכן הריצה שכתבה min-stay הותירה 32 שורות no-op ואפס עקבות של שינוי ההגבלה. הערך הקודם
  **כן** נקרא בקוד (`src/lib/rates/service.ts`) ואז נזרק.

**סגירת WS-4 — ה-rate plans של Booking.com (בעלים, 12/09/2026).** Booking.com rate plans
62626705 'Monthly rate' and 58374173 'Weekly rate' are Booking-side derived plans (percentage
discounts the owner sets in the extranet on top of Standard rate). GuestHub intentionally manages
only the Standard rate (price1 → rateCode 58346725); these two are out of scope and must not be
mapped or deleted (owner, 12/09). הרקע: ‏GH-BATCH-1 WS-4 מצא ששני המזהים אינם קיימים באף משטח
GET של Beds24 (‏/properties, ‏/inventory/fixedPrices, calendar) — הם חיים באקסטרנט של Booking בלבד.

## D184 — הפרות הגבלת-שהות של OTA מוצגות בפאנל "הזמנות ערוץ שנתקעו" עד סימון "טופל" (2026-09-11)

**ההכרעה (בעלים, 11/09/2026 — D183 הכרעה 3).** הזמנה שיובאה מערוץ ומפרה הגבלת שהות של GuestHub
(שורת `channel_sync_errors` עם `error_code = 'OTA_STAY_RESTRICTION_VIOLATION'`, למשל `MIN_STAY_NOT_MET`)
מופיעה בפאנל הדשבורד **הקיים** "הזמנות ערוץ שנתקעו" — אותו פאנל, בלי מסך חדש — עם סימון בולט, עד
שמפעיל מסמן אותה "טופל". שורות drift של ה-read-back **אינן** חלק מזה: הן תאים, לא הזמנות, ונשארות
במסך הסנכרון.

**מה מצא האודיט (11/09), ולמה זה משנה את הצורה.** הפאנל אינו רשימה — הוא **מונה** (`StuckWindow.tsx`:
"A COUNTER, not a list", D169): `count(DISTINCT provider_booking_id)` על `channel_booking_revisions`
עם `import_status IN ('quarantined','failed')`. רוב הרוויזיות התקועות מעולם לא הפכו להזמנה (הסגר =
אין מיפוי = אין `reservations` row), ולכן אין להן צורת הזמנה להציג. הטבלה `channel_sync_errors` **לא**
מכילה `kind`/`reservation_id`/`resolved_by` — הקישור להזמנה הוא `context->>'reservation_id'`, הסוג הוא
`error_code`, ו-`resolved_at` נכתב עד היום רק אוטומטית (סוגר ה-read-back מ-086, סוגר ההסגר מ-D164/D167).
אף UI לא הציע "סמן כטופל" — הרשימה ב-/channels היא קריאה בלבד (10 שורות פתוחות אחרונות).

**המימוש — שני מקורות, פאנל אחד.**
- **מודל הקריאה** (`src/lib/channel/stuck-panel.ts`, שני השאילתות של הפאנל, `db` מוזרק כדי שהשומר
  יריץ אותן): `loadStuckCounter` — המונה של D169, ללא שינוי; `loadStuckViolations` — שורות
  `OTA_STAY_RESTRICTION_VIOLATION` פתוחות (`resolved_at IS NULL`) של הטננט, ב-JOIN פנימי להזמנה דרך
  `context->>'reservation_id'` (עם שומר regex ל-uuid), נושאות מספר הזמנה, אורח, חדר ראשון, תאריכים,
  לילות, ערוץ מקור (`ota_name`), קודי המנוע והמשפט העברי מ-`context.violations` (מאוחדים, ללא כפילות;
  נסיגה ל-`error_message`), `created_at`, ושני סימונים: `stuck` (לאותה הזמנה יש רוויזיה תקועה פתוחה —
  דרך `local_reservation_id` **או** `connection_id + provider_booking_id`) ו-`cancelled`. חדש-ראשון.
  הזמנה שהיא **גם** תקועה **וגם** מפרה מופיעה פעם אחת — כשורת הפרה עם התג "תקועה"; המונה סופר את
  ההזמנה שלה כפי שספר תמיד. הזמנה מבוטלת **נשארת** ברשימה עם תג "בוטלה" — הפגם הוא בהקרנה, לא בהזמנה.
- **UI** (`StuckWindow.tsx`, `dashboard-widgets.css`): מתחת למונה (או במקומו, כשהוא 0) רשימת שורות
  באנטומיה של `.pay-row`: אייקון בטון danger, גוף מקושר ל-`/reservations?open=<id>` עם שלוש שורות —
  שם אורח (+תגים), **הסמן** `.stk-vio-mark` ("מפרה מינימום 2 לילות בטווח זה", טון danger נגזר מהטוקן כמו
  `.pay-tag-cxl`, עוטף ולא נחתך), ושורת משנה ("#1164 · Booking.com · חדר 1245 · 11/9–12/9 · לילה
  אחד") — וכפתור "סמן כטופל". ב-390px הכפתור עוטף מתחת לגוף (`flex-wrap`, בסיס 200px), נמדד: אפס
  גלישה אופקית. מצב-ריק ללא שינוי, ומופיע רק כששני המקורות ריקים.
- **הכיתוב בכותרת** כולל את ההפרות: "N תקועות" (ללא שינוי) / "N הפרות" / ובשניהם — **איחוד** "N לטיפול"
  (תקועות + מפרות-שאינן-תקועות). כיתוב דו-חלקי נמדד 138px ודחף את ה-✕/גריפ לשורה שנייה בעמודת
  הדסקטופ (404px); ב-390 הכותרת עוטפת בכל וריאנט, כולל הישן — התנהגות `.card-hd` המתועדת.
- **הפעולה** `resolveSyncErrorAction(errorId)` ב-`src/lib/channel/admin.ts`: `requireChannelAdmin`
  (super_admin — אותו שער של כל פעולה על `channel_sync_errors`; /channels עצמו חסום למי שאינו
  מנהל-על), `UPDATE … SET resolved_at = now(), resolved_by = <actor> WHERE id AND tenant_id AND
  resolved_at IS NULL RETURNING id` — אידמפוטנטית (לחיצה שנייה = 0 שורות, מחבר הסגירה הראשון נשאר),
  תחומה לטננט, **לעולם לא מחיקה** (השורה היא היסטוריה; purge 043 מסירה אותה 30 יום אחרי הסגירה).
  הכפתור מוצע רק למי שהשרת יקבל (`canResolveViolations` מהעמוד), והשרת בודק בכל מקרה.
- **מיגרציה 088** — `channel_sync_errors.resolved_by uuid REFERENCES users ON DELETE SET NULL`.
  ‏`NULL` = סגירה אוטומטית (הסוגרים לא השתנו), ערך = מפעיל. ללא backfill.

**RBAC (לא נוספו תפקידים).** הדשבורד נראה לכל מחובר (אין שער — יעד ההפניה של כל דחייה, audit §2.4);
"סמן כטופל" — super_admin בלבד, כמו /channels.

**הוכחה.** `check:stuck-panel-violations` (מסד scratch משלו על :5433, שרשרת המיגרציות המלאה כולל 088,
המודול האמיתי מקומפל): fixture של תקועה-בלבד, מפרה-בלבד, שניהם (מקושרת רק דרך connection+booking),
נסגרה, טננט זר, context פגום, והזמנה שנמחקה — מונה 2/5 שעות; 3 שורות חדש-ראשון עם הסימונים הנכונים;
הסגורה, הזרה, הפגומה והיתומה מחוץ; predicate הסגירה: 0 שורות לטננט זר, 1 לבעלים, 0 בלחיצה שנייה,
`resolved_by` = המפעיל, השורה לא נמחקה. ‏B2: נטרול ה-EXISTS של `stuck` → 1 טענה אדומה, exit 1;
מחיקת `resolved_at IS NULL` → 3 אדומות, exit 1; שחזור → 7/7. צילומי CDP על ה-CSS האמיתי (הקומפוננטות
האמיתיות מרונדרות ב-react-dom/server): `docs/proof/d184-stuck-panel-{390,1280}[-card].png` — שורת
1164 החיה, הסמן גלוי, `scrollWidth == viewport` בשני הרוחבים.

**ספירה חיה (SELECT בלבד, 11/09).** שורת הפרה אחת בכל הטבלה, פתוחה: הזמנה 1164 (Booking.com, חדר 1245,
‏11–12/09, `checked_in`, "מינימום 2 לילות בטווח זה"); 0 רוויזיות תקועות. אימות אחרי פריסה (בעלים):
1164 מופיעה בפאנל, "סמן כטופל" מסיר אותה.

**מה לא נעשה, במכוון.** המונה לא הפך לרשימה — זו הכרעת D169 שאיש לא ביטל, ורוב התקועות אינן הזמנות.
...
## D189 — טבלת אמצעי התשלום נחתכה ברוחב טאבלט: הגריד הפך fluid (2026-09-12)

`.pm-tbl { min-width: 780px }` דחף את עמודת המתג אל מעבר לקצה הכרטיס מאחורי סרגל גלילה כשהסיידבר פתוח (הכרטיס ~590px ב-900, ~690px גם ב-1280 בשני-הפאנלים); הרצפה הוסרה, עמודת המפתח היא `minmax(56px, 130px)` והצ׳יפ נחתך באליפסיס עם `title` של המפתח המלא, עמודת השימוש מתכווצת (השבירה מותרת), תא המתג `min-width: 44px`; בטווח 768–1119 מסלולים קומפקטיים (44/32/64, ‏padding 12). ‏CSS + ‏`title`/קלאס בלבד, ללא שינוי DOM; נמדד ב-CDP: 900 (סיידבר 250) → הטבלה 586=כרטיס, ללא גלילה, כל המתגים בפנים; 768/250 → 454=454; 390 ללא שינוי. שומר: `check:responsive` ‏§15 (B2: החזרת הרצפה / עמודת מפתח קבועה → יציאה 1).

## D190 — טבלאות הסטטוסים והמנעולים נחתכו ברוחב טאבלט: שני הגרידים הפכו fluid (2026-09-12)

אותו פגם ואותו מתכון כמו D189, בשני הקבצים האחים: `.ws-tbl { min-width: 830px }` (‏status-settings.css) ו-`.lk-gt { min-width: 900px }` (‏locks.css) דחפו את עמודות הכוכב/הפעולות אל מעבר לקצה הכרטיס מאחורי סרגל גלילה כשהסיידבר פתוח (סטטוסים: כרטיס 586 ב-900, ‏692 גם ב-1280 בשני-הפאנלים; מנעולים: 590 ב-900, ‏714 ב-1024 — ושם גם כפתור ההעתקה של תא הקוד נחתך, והתא התשיעי הריק גלש ב-24px של padding בכל רוחב, כולל 1280 עם סיידבר מכווץ). הרצפות הוסרו. סטטוסים: `52px 40px minmax(88px,1.3fr) minmax(64px,1fr) 80px 64px minmax(80px,.9fr)`, הצ׳יפ נחתך באליפסיס עם `title` של התווית המלאה, תא המתג `min-width: 44px`, ‏`.ws-acts` נשבר (מצב אישור-מחיקה), בטווח 768–1119 מסלולים קומפקטיים (44/32/64/56, ‏padding 12). מנעולים: המקסימומים של הרפרנס נשמרו והרצפות ירדו ל-805px סה״כ (‏≤822 = 1120 עם סיידבר פתוח); בטווח 768–1119 סט קומפקטי של 580px — ‏padding תא 8, כפתורי הקוד נשברים מתחת לקוד (`.lk-codebox { flex-wrap: wrap; max-width: 100% }`), התג/הסוללה/כותרות העמודות רשאים להישבר; תא הבחירה `min-width: 44px`, תא הפעולות `min-width: 84px`, ל-`.lk-room` נוסף `title`, לתאי ה-spacer הריקים `padding: 0`. ‏CSS + ‏`title`/קלאס בלבד, ללא שינוי DOM. נמדד ב-CDP על ה-CSS האמיתי (DOM 1:1, סיידבר 250/76): סטטוסים 900 → 586=586, ‏1024 → 710=710, ‏1280/250 → 692=692, ‏768 → 454=454; מנעולים 900 → 590=590, ‏1024 → 714=714, ‏1280/76 → 1144=1144 (היה 1168), ‏1280/250 → 970=970; ‏390 זהה לפני/אחרי בשניהם. **גבול ידוע:** ב-768 עם סיידבר פתוח (כרטיס 458) גריד המנעולים בן 9 העמודות עדיין גולל (580 > 458); עם סיידבר מכווץ (644) הוא נכנס. שומר: `check:responsive` ‏§16/§17 (B2: החזרת רצפה / הסרת `flex: none` מתא המתג או הפעולות → יציאה 1).

## D191 — בסיס ה-diff של הפריסה נקרא מקובץ מצב (`.deploy-state`), לא מ-HEAD; חסין ל-`git pull` ידני (2026-09-12)

**התקרית (12/09/2026, פריסה ‎#1).** D185 גזר את "הקומיט שנפרס קודם" מ-HEAD של עץ העבודה לפני ה-fetch. ‏`git pull origin main` ידני רגע לפני `deploy:prod` (הדפוס המתועד בהיסטוריית השל: `git pull origin main && PROD_DEPLOY_OK=1 npm run deploy:prod`) כבר הזיז את HEAD ליעד — הטווח קרס ל-HEAD..HEAD, הודפס "dependencies unchanged", וההתקנה דולגה בשקט לריליס ששינה `package.json`.

**ההוראה (בעלים).** הסקריפט כותב `.deploy-state` (‏gitignored, בשורש `/var/www/guesthub`, ‏JSON ‏`{commit, build, at}`) בסוף פריסה מוצלחת, וקורא ממנו את `commit` בתחילת הפריסה הבאה כבסיס ה-diff. קובץ חסר → ההתנהגות הקודמת (HEAD) + שורת לוג רועשת `no .deploy-state — base derived from HEAD`. ‏HEAD ≠ ה-`commit` שבקובץ (מישהו משך ידנית) → WARNING בלוג, והבסיס הוא עדיין הקומיט שבקובץ.

**המימוש.** `scripts/deploy-state.mjs` (חדש): `read <file> <head>` מדפיס את הבסיס ל-stdout ואת השורות הרועשות ל-stderr; קובץ שקיים אך אינו JSON תקין / אינו נושא sha מלא / מצביע על קומיט שאינו בריפו → יציאה 1 והפריסה נעצרת (לעולם לא בסיס מנוחש). `write <file> <commit> <build>` כותב אטומית (tmp + rename). ‏`scripts/deploy-production.sh`: `HEAD_COMMIT` נלכד; `BEFORE_COMMIT="$(… read "$DEPLOY_STATE_FILE" "$HEAD_COMMIT")" || fail` לפני ה-fetch; שורת ה-before מדפיסה `deployed=… head=…`; שלב 14 — הכתיבה — הוא השורה האחרונה בסקריפט, אחרי ה-restart, בדיקות הפורט והמסלולים ושורת ההצלחה, כך שפריסה שנכשלה בכל שלב משאירה את הרשומה הקודמת; כישלון כתיבה מדפיס אזהרה ויוצא 1 (הפריסה עצמה כבר חיה; הבאה תיפול ל-HEAD בקול). ‏`.deploy-state` נוסף ל-`.gitignore` (שומר הפריסה ממילא מתעלם מ-`??`).

**Seed.** אין קובץ עדיין: הריצה הראשונה אחרי המיזוג נופלת ל-HEAD עם השורה הרועשת — כלומר מתנהגת בדיוק כמו D185 — וכותבת את הקובץ בסופה; מהפריסה השנייה ואילך הבסיס מגיע מהקובץ.

**הוכחה יבשה על ההיסטוריה האמיתית** (קובץ מצב זמני, ללא פריסה). (א) כפי שהוגדר בתדריך: seed ‏`commit=98664a0`, ‏HEAD=`7b6abe7` אחרי "pull ידני" → WARNING, ‏base=98664a0, טווח `98664a0..7b6abe7`, ‏`changedDependencyFiles = []` — **בטווח הזה `package.json` לא השתנה** (‏#258/#260 נגעו רק ב-CSS/TSX/DECISIONS/שומר; הנחת התדריך "it did change" אינה נכונה לטווח הזה — נבדק ב-`git diff --name-only 98664a0 7b6abe7 -- package.json pnpm-lock.yaml`; המנגנון עצמו הוכח: הבסיס הגיע מהקובץ ולא מ-HEAD). (ב) אותו מנגנון על טווח הפריסה של ‎#255 שבו `package.json` כן השתנה: seed ‏`commit=f439fb6`, ‏HEAD=`2103a6b` → base=f439fb6, ‏`["package.json"]` מזוהה; ‏HEAD..HEAD — מה ש-D185 לבדו היה משווה אחרי pull — `[]`.

**שומר.** `check:deploy-script` — מבנה: `HEAD_COMMIT` → `DEPLOY_STATE_FILE` → `BEFORE_COMMIT` דרך `deploy-state.mjs read` עם `|| fail`, לפני ה-fetch; אף שורת `BEFORE_COMMIT="$(git rev-parse HEAD)"`; הכתיבה אחרי ה-restart, לולאת המסלולים ושורת ההצלחה, פעם אחת בלבד; `.deploy-state` ב-`.gitignore`. התנהגות (ריפו סקראץ' A→D): חסר → HEAD + השורה הרועשת; קיים → הרשומה; HEAD זז → WARNING + הרשומה, ושינוי המניפסט מזוהה במקום ש-HEAD..HEAD מצא כלום; JSON שבור / קומיט לא-קיים → זריקה; CLI ‏read/write. B2: קריאה מ-HEAD גם כשהקובץ קיים → יציאה 1; כתיבה לפני ה-restart → יציאה 1; הסרת `.deploy-state` מ-`.gitignore` → יציאה 1; שחזור → 7/7.

## D192 — חלון "דורש טיפול" (`alr`) הוסר מהדשבורד עד אפיון מחדש; הקוד והשאילתה נשארים (2026-09-13)

**ההוראה (בעלים).** החלון "דורש טיפול" לא יוצג לאף משתמש עד שיאופיין מחדש — הסרה, לא הסתרה. ה-× של הדשבורד אינו מתאים למטרה: הוא per-user ב-`users.ui_preferences` (מיגרציה 072), נשמר לצמיתות, וכל משתמש מחזיר חלון מוסתר בקליק מפס "חלונות מוסתרים". הדרך שנבחרה (אודיט קריאה-בלבד באותו יום, אפשרות א׳ + תיקון הקומפילציה): הסרת הרשומה `alr` מ-`WINDOWS` ומ-`DEFAULT_ORDER.r` ב-`windows.ts`, והסרת `case "alr"` והייבוא של `AlertsWindow` ב-`DashboardScreen.tsx`. שני הקבצים באותו קומיט בהכרח — ההסרה מה-registry מצמצמת את `DashboardWindowId`, ו-`case "alr"` על איחוד שאינו מכיל אותו הוא שגיאת TS2678 (הוכח עם ה-tsc של הריפו לפני הביצוע). `_everyWindowIsPlaced` ו-`buildDefaultLayout()` לא שונו — הם מה שכופה את הסימטריה בין שני המקומות.

**מה נשאר בכוונה.** `AlertsWindow.tsx`, הטיפוס `AlertRow`, הפונקציה `dashboardAlerts` (data.ts) ומחלקות ה-CSS `.alr-*` נשארים ללא שינוי — הקוד שמור לאפיון מחדש. `dashboardAlerts` ממשיכה לרוץ בכל טעינת דשבורד (חמש שאילתות) ותוצאתה `data.alerts` אינה מרונדרת; זה מחיר מודע, ותמורתו שני השומרים `check:alerts-window-includes-cancelled` ו-`check:connection-health` נשארים חיים ואוכפים על הקוד המשומר. מחלקות `.alr-icon/.alr-red/.alr-amber/.alr-ok` ממילא משותפות ל-`iss`/`pay`/`stk`. אין מיגרציה ואין נגיעה ב-DB: `normalizeDashboardLayout` זורק מזהה לא-רשום מה-layout ומ-`hidden` בכל קריאה ובכל כתיבה (הבדיקה `ghost` ב-`check:dashboard-shell`), כך ששורות `ui_preferences` שמכילות `"alr"` הן JSON ישן חסר-השפעה שייכתב מחדש בגרירה הבאה של אותו משתמש.

**השלכות על הכרעות קודמות.**
- **D140:** הבית של טיוטה עם חוב היה חלון "דורש טיפול" (כפתור האישור draft→confirmed). מעתה הבית היחיד שלה הוא מסך ההזמנות (אותו מעבר draft→confirmed שם). נימוק D140 להחרגת `draft` מחלון החיובים (`pay`) עומד בעינו — טיוטה אינה הזמנה שלא שולמה אלא הזמנה שטרם נוצרה.
- **D141:** הקפאת שאילתת הכסף של `dashboardAlerts` (החרגת `approved` על ציר הוורקפלואו + `draft` על ציר ה-lifecycle, `cancelled` בפנים ומתויג) נשארת אכופה ע"י `check:alerts-window-includes-cancelled` על שאילתה שאינה מוצגת.
- **D133:** ורדיקט ששת תנאי בריאות החיבור (`connectionHealth`) הוצג אך ורק בחלון הזה; עד האפיון מחדש אין לו UI. סיכון ידוע ומקובל לתקופת הביניים; הכלל, השאילתה שמזינה אותו והשומר שלהם נשארים.

**שומר.** `check:dashboard-shell` עודכן באותו קומיט בלי להחליש אותו: 11 חלונות רשומים, סדר ברירת המחדל של עמודה ימין מתחיל ב-`pay`, קלט בדיקת הכפילות הוא `r: ["msg", "pay"]` (מזהה חי, כדי שהבדיקה תישאר משמעותית); בדיקת ה-`ghost` לא השתנתה.

## D193 — `gen-catalog.sh` אינו דורס את `CLAUDE.md`: הקיט הוא source of truth לקטלוג, ו-`AGENTS.md` הוא התוצר (2026-09-18)

**הסתירה.** שתי אמירות חיות ישבו בריפו זו מול זו. `CLAUDE.md:120` (= `AGENTS.md:105`, בסעיף OMX Runtime) קבע ש"התבניות של ה־KIT הן ה־source of truth ל־`CLAUDE.md` ו־`AGENTS.md`" — כלומר ש-`CLAUDE.md` הוא תוצר מרוגנר. `D90 §1` קבע את ההפך: "`CLAUDE.md` = הבית הקנוני... מתוחזק ידנית/ב-`/init` בתוך הריפו **ואינו נדרס** ע"י מחולל הקטלוג". ההכרעה קריטית מפני שכללי ה-git (main מוגן, זרימת PR, שלושת ה-status checks), כללי ה-CI והיררכיית התיעוד שנוספו בספטמבר 2026 יושבים כולם ב-`CLAUDE.md`: אם התבנית דורסת אותו, כולם חשופים למחיקה שקטה ברגנרציה הבאה.

**למה נדרשה ראיה ולא קריאה במחולל.** `gen-catalog.sh` והתבניות חיים ב-`~/DevOPS/` על ה-hub `ai2u-vs1` ואינם נגישים מהשרת הזה (המראה המקומית `/home/ubuntu/DevOPS/` כבר לא קיימת; חיבור ל-hub היה מחוץ להיקף). ההכרעה נשענת אפוא על ראיות תצפיתיות בלבד — מה המחולל **עשה בפועל** על הדיסק, לא מה שהתיעוד טוען עליו.

**הראיות (11, כולן קריאה בלבד).**
1. **סוויפ 22/07/2026 15:02** — `find` על כל קובץ בשרת שנכתב בדקה הזו מחזיר 8 קבצים: `sea-tower/AGENTS.md`, `pms/AGENTS.md`, `sys/AGENTS.md`, `pms-build/AGENTS.md`, `mail-system/AGENTS.md`, `pms/claude/AGENTS.md`, `pms-build/claude/AGENTS.md`, `pms/.claude-md-backups/20260420_035612/AGENTS.md`. **כולם `AGENTS.md`; אף `CLAUDE.md` אחד.**
2. **תיקיית הגיבוי** — הסוויפ נכנס גם ל-`pms/.claude-md-backups/20260420_035612/` ורענן שם את `AGENTS.md`, בעוד `CLAUDE.md` ו-`CLAUDE.md.bak` באותה תיקייה נשארו על 07/06 14:34. המחולל סורק תיקיות ללא הבחנה ועדיין לא נגע ב-`CLAUDE.md` — אין כאן מקריות של מיקום.
3. **mtime מול mtime** — `sys`/`mail-system`: AGENTS ב-22/07 15:02 מול CLAUDE ב-07/06; `pms`: מול 30/06; `sea-tower`: מול 17/08. ה-`CLAUDE.md` נערכו לפני הסוויפ ואחריו ולא הושפעו ממנו.
4. **מבנה הקובץ** — ב-`sys`, `mail-system` ו-`pms`, `AGENTS.md` הוא **בייט-בבייט ה-`CLAUDE.md` + 160 שורות** (`head -n $(wc -l < CLAUDE.md) AGENTS.md | cmp - CLAUDE.md` עובר בשלושתם).
5. **160 השורות הנספחות** — md5 `73b5879e132aad21`, זהה בכל שבעת הפרויקטים שנבדקו (`sys`, `mail-system`, `pms`, `sea-tower`, `guesthub`, `billing`, `invoice`). זהו קטלוג ה-skills/agents ("נוצר אוטומטית — 97 skills, 50 agents") — החלק היחיד שהקיט באמת מכתיב.
6. **תוכן פר-פרויקט עובר לשני הקבצים** — `sys` נושא "Project Root: `sys.bios.co.il` תחת `/var/www/sys`"; `mail-system` נושא "Restart Dev After Changes... פורט 3003, `mail.bios.co.il`". כל אחד מהם מופיע גם ב-`CLAUDE.md` וגם ב-`AGENTS.md` של אותו פרויקט. תבנית משותפת אחת אינה יכולה לייצר את זה.
7. **ראיית הקצה — `sea-tower`** — ה-`CLAUDE.md` שלו אינו "Master Orchestrator" כלל אלא `# Sea Tower — Project Rules`. ה-`AGENTS.md` שלו נפתח באותו טקסט בדיוק ומסתיים באותו קטלוג משותף. מכריע: הגוף מגיע מהפרויקט, הזנב מהקיט.
8. **`git log --follow --stat -- CLAUDE.md` ב-guesthub** — 10 קומיטים, כולם ממוקדים וענייניים (`+13`, `+2`, `+2`, `+66`, `+8/-5`, `+8`, `+2/-1`, `+13`, `+19`, ו-`+121` בצ'ק-אין הראשוני `0092f60`). אין אף קומיט של דריסה מסיבית בסגנון רגנרציה.
9. **`git log` ב-`sea-tower`** (ריפו בלתי תלוי, קריאה בלבד) — ל-`CLAUDE.md` שלושה קומיטים ידניים בלבד. אותה תמונה.
10. **המכניקה של D90 עצמו** — ב-`12ed557` (21/07) סעיף ה-Concurrency נוסף **רק ל-`AGENTS.md`**. הרגנרציה שבאה אחריו מחקה אותו; ה-diff של `d66a49b` מתעד את הרגנרציה בכך שמונה הקטלוג קופץ בו `92 skills, 45 agents` → `97 skills, 50 agents`. תואם במדויק: `AGENTS.md` נבנה מ-`CLAUDE.md`, שלא הכיל אז את הסעיף.
11. **הקוהרנטיות של הפתרון ב-D90** — התרופה שנבחרה שם ("לשים את הסעיף ב-`CLAUDE.md` כדי שישרוד") עובדת **רק** אם `CLAUDE.md` אינו נדרס והוא הקלט לרגנרציה.

**ההכרעה.** `gen-catalog.sh` כותב אך ורק ל-`AGENTS.md`, ובונה אותו מ**גוף ה-`CLAUDE.md` של הפרויקט + קטלוג ה-skills/agents מהקיט**. הקיט הוא ה-source of truth **לקטלוג בלבד** — לא ל-`CLAUDE.md`, ואף לא לגוף של `AGENTS.md`. `D90 §1` נכון; `CLAUDE.md:120` הוא שהיה שגוי.

**המשמעות לתוכן שיושב ב-`CLAUDE.md` — הפוכה מהחשש.** כללי ה-git, ה-CI והיררכיית התיעוד יושבים ב-`CLAUDE.md`, שהוא **המקום הבטוח היחיד**. החשיפה היא בדיוק ההפך: כלל שנכתב **רק** ב-`AGENTS.md` יימחק ברגנרציה הבאה בלי אזהרה. זה מה שקרה ל-D90, וזה מה ששלושת השומרים הקיימים — `check:agents-concurrency`, `check:agents-rule12`, `check:agents-worktree` — מגנים מפניו. **אין צורך בשומר חדש ל-`CLAUDE.md`**, ומכאן גם שכל כלל חדש נכתב ראשית ב-`CLAUDE.md` ורק אז משתקף ל-`AGENTS.md`.

**מה תוקן.** שלוש שורות שנשאו את אותו הסבר שגוי, ושום כלל לא נגע: (א) `CLAUDE.md:120` = `AGENTS.md:105` — ההוראה התפעולית (`לא מריצים omx agents-init .`) נשארה, והנימוק שאחריה הוחלף בתיאור הנכון; (ב) הערת השוליים של D90 בשני הקבצים, שייחסה את המחיקה ל"תבנית שלא כוללת את הסעיף" במקום למנגנון האמיתי; (ג) הערת הראש של `scripts/check-agents-worktree.mjs`, שטענה ששני הקבצים מרוגנרים — לוגיקת השומר (`REQUIRED`/`FORBIDDEN`) לא שונתה, והדרישה שנוסח D147 יופיע בשני הקבצים נשארת נכונה דווקא מפני ש-`CLAUDE.md` הוא הקלט לרגנרציה הבאה.

**ה-Action item הפתוח של D90 מיותר.** הסעיף שם ("להוסיף את סעיף ה-Concurrency לתבנית ב-hub `ai2u-vs1`") נוסח תחת ההנחה שהתבנית מכתיבה את גוף `AGENTS.md`. לפי הראיות הוא אינו נדרש: הסעיף כבר יושב ב-`CLAUDE.md`, שהוא הקלט לרגנרציה, ולכן כל רגנרציה עתידית תשחזר אותו ל-`AGENTS.md` מעצמה. **טקסט D90 עצמו לא נערך** — הרשומה נשארת כלשונה כתיעוד היסטורי, וההכרעה על המנגנון יושבת כאן.

## D194 — בדיקות GuestHub נכתבות כגארדי `check:*` בסוויטה — לא vitest ולא jest (2026-09-25)

**ההקשר.** בחקירת באג התמחור באתר (D195) דווח בטעות ש"ל-GuestHub אין מסגרת בדיקות", ועל סמך זה אושר PR מקדים שמוסיף vitest. הדיווח תוקן לפני שנפתח ענף: לריפו יש סוויטת גארדים — ‏`check:*` ב-`package.json` (‏147 סקריפטים), רץ דרך `scripts/run-checks.mjs` שמעניק לכל גארד DB מבודד משוכפל מתבנית שנבנית מהמיגרציות (‏`guesthub-testdb`, פורט 5433 — לעולם לא 5432), ו-`suite` הוא status check חובה ב-CI לכל PR. קיים גם תקדים ישיר לתפר הזמינות/מנוע: `check:public-quote`.

**ההכרעה (בעלים).** ‏PR ה-vitest בוטל. לא מוסיפים מסגרת בדיקות חיצונית (‏vitest/jest או דומה). בדיקות ב-GuestHub נכתבות כגארד `check:*` בסוויטה הקיימת: סקריפט `scripts/check-<name>.mjs` + מפתח `check:<name>` ב-`package.json` (הסדר ב-`package.json` הוא סדר הריצה), לפי תבנית הגארדים הקיימים — קימפול המודולים האמיתיים דרך `tsc` ל-CJS עם stub ל-`server-only`, ‏fixture בטרנזקציה שמתגלגלת לאחור, ‏`./lib/collect-assert.mjs` (D127), סירוב לכל DSN עם סמן פרודקשן.

**למה.** (א) גארד חדש רץ ב-CI מיד כחלק מ-`suite`, בלי job ובלי חיווט נוסף; מסגרת מקבילה לא הייתה רצה בשום מקום עד שמחברים אותה. (ב) ה-DB המבודד לכל גארד הוא בדיוק מה שבדיקת תמחור צריכה, וכבר קיים. (ג) הריפו ציבורי עם גארד supply-chain — תלות dev חדשה היא שטח תקיפה ותחזוקה שאין לו הצדקה. (ד) גארד אחד לכל אינווריאנט הוא הדפוס שכל ההכרעות הקודמות (D106, D127, D147, D159…) נשענות עליו.

**כלל נלווה (בעלים, מאותו יום).** מספרים צפויים בגארד תמחור מחושבים מהשורות שהגארד טוען (הגדרות הנכס, עמודות החדר, התעריפים) דרך אותן פונקציות טהורות שהמנוע מריץ — לא מספר קבוע ב-assertion. ראה `scripts/check-public-availability-guests.mjs` (D195) כתבנית.

## D195 — ‏`/api/public/availability` מתמחר לפי הרכב האורחים (`guests`) דרך המנוע; יחידה מוצעת אם היא מתאימה לפחות לאחד ההרכבים, וההזמנה מקצה חדרים לפי `partyPrices[i]` (2026-09-25)

**הבאג.** האתר (sea-tower) שלח ל-`/api/public/availability` תאריכים בלבד. ‏`publicAvailability` תמחר במנוע רק את היחידה הזולה בכל סוג, ותמיד ל-2 מבוגרים; שאר היחידות חזרו עם סכום ה-ESS הגולמי. ההזמנה (`createPublicBooking`) תמחרה את ההרכב האמיתי ודחתה כל `expectedTotal` שסטה ביותר מ-₪1. בלוג nginx: כל 6 ניסיונות ההזמנה מהאתר ב-23–24/09/2026 נכשלו ב-409 — שניים ב-`price_changed` (דירה 1318, ‏2 מבוגרים + 3 ילדים: החיפוש הציג ₪1,500 והמנוע דרש ₪2,700), וארבעה ב-`no_availability` (דירה 1235, ‏2+2: סוג החדר מצהיר תפוסה 4 אבל היחידה עצמה 3 — האתר סינן לפי `maxOccupancy` של הסוג). אף הזמנה לא נסגרה במחיר חסר (כל 11 הזמנות האתר ב-DB היו ל-2 מבוגרים).

**הכרעות הבעלים (25/09/2026).** (א) שרשרת הירושה — חדר, ואם אין ערך אז הנכס (`resolveEffectivePricing`) — היא מקור האמת לתפוסה ולתוספות; לא משנים אותה ולא נוגעים בנתוני החדרים ב-DB (‏1130/1131 בחריגה חלקית נשארים כפי שהם; הבעלים מעדכן דרך `/rooms` אם ירצה). (ב) כל סכום ב-availability עובר דרך `resolveEffectivePricing` והמנוע, כולל הגדרות הנכס (עיגול, ספירת תינוקות, גילאים) — כך שכל שינוי ב-`/rooms` או בהגדרות הנכס חל באתר מיד. (ג) אין מחיר או תפוסה מקובעים בקוד או בבדיקות. (ד) קודם GuestHub, אחר כך האתר; פריסה לפרודקשן רק באישור. (ה) **רב-חדרי (תיקון מאותו יום, לפני merge):** הגרסה הראשונה של ה-PR הציעה יחידה רק אם התאימה להרכב *הראשון* — לקוח שמחפש חדר ראשון 2+2 וחדר שני 2+0 לא ראה יחידות קטנות שמתאימות רק לחדר השני, ולכן לא קיבל שילוב תקין. הבעלים דחה זאת: יחידה מוחזרת אם היא מתאימה לפחות לאחד ההרכבים; `partyPrices[i]` מכיל מחיר מהמנוע לכל הרכב שהיא מתאימה לו ו-`null` לכל הרכב שאינה; יחידה שאינה מתאימה לאף הרכב אינה מוחזרת; יצירת ההזמנה מאמתת שכל יחידה שהוקצתה לחדר `i` מתאימה להרכב `i`, ואם לא — שגיאה מפורשת.

**החוזה.** `GET /api/public/availability?check_in&check_out[&guests=2-0,3-1]` — `guests` בפורמט האתר: חדר לכל פסיק, `מבוגרים-ילדים` לחדר (מבוגרים 1–6, ילדים 0–4, עד 5 חדרים — אותם גבולות של `POST /api/public/bookings`; הפרסר היחיד: `src/lib/public-booking/guests.ts`). ערך לא תקין → 400 `validation`. עם `guests`: לכל יחידה `partyPrices` — מערך מיושר לרשימת `guests`, ‏`partyPrices[i]` = מחיר המנוע ליחידה הזו תחת ההרכב של חדר `i` (תוספות אורחים כלולות), ‏`null` = היחידה אינה יכולה לארח את ההרכב הזה (קיבולת/תפוסה); יחידה מוצעת אם לפחות ערך אחד הוא מספר, ויחידה שאינה מתאימה לאף הרכב אינה מוחזרת. ‏`totalPrice` של היחידה = הערך הראשון שאינו `null` (המחיר לחדר הראשון שהיא יכולה לארח; בחיפוש של חדר אחד זהו פשוט המחיר להרכב). ‏`availableUnits`, ‏`totalPrice`/`pricePerNight`/`nightly` של הסוג נגזרים מהיחידות המוצעות בלבד; המיון — `totalPrice` ↑ ואז קוד; התשובה מהדהדת `guests` בצורתו הקנונית. בלי `guests`: התשובה זהה למה שהייתה (‏2 מבוגרים, הזולה דרך המנוע, השאר ESS; ‏`guests: null`, בלי `partyPrices`) — BIOS Bot ממשיך להרכיב על זה.

**הקצאת חדרים בהזמנה (`src/lib/public-booking/assign-units.ts`, טהור).** `createPublicBooking` מריץ את אותה `publicAvailability` עם אותם `parties` (‏`input.rooms`, תינוקות 0) ואז `assignUnitsToRooms(type.units, rooms.length, preferredUnitId)`: (1) הדירה שנבחרה בכרטיס משרתת את חדר 1 — היא חייבת להיות מוצעת **ו**לארח את הרכב 1, אחרת 409 `unit_party_mismatch` (קוד חדש ב-`PublicBookingError`), לעולם לא החלפה שקטה; (2) שאר החדרים מקבלים את **השילוב התקין הזול ביותר** מהיחידות שנותרו — מינימום של Σ`partyPrices[i]` על החדרים, כל יחידה לכל היותר פעם אחת (שוויון: השילוב שנמצא ראשון כשמנסים יחידות לפי סדר התשובה, חדר אחרי חדר); אין שילוב → 409 `no_availability` עם הודעה מפורשת. עם הרכבים שווים זו בדיוק בחירת "N היחידות הזולות" הקודמת. אחרי ההקצאה — הבדיקה המפורשת שדרש הבעלים: לכל `i`, ‏`picked[i].partyPrices[i]` הוא מספר, אחרת `unit_party_mismatch`. אין יותר חיתוך מיקומי (`slice`) של רשימת היחידות. האתר (PR 2) משקף את אותו כלל כדי לחשב `expectedTotal`, ומוסיף הודעה לקוד `unit_party_mismatch` (עד אז הוא נופל להודעה הגנרית).

**המימוש.** `publicAvailability(db, in, out, { parties })` → `priceUnitsForParties`: קריאת מנוע אחת לכל הרכב **שונה** על כל היחידות הזמינות (המנוע דוחה `roomId` כפול בבקשה אחת — `ROOM_DUPLICATED` — ולכן הזמנה רב-חדרית עם שני הרכבים היא שתי קריאות, לא קריאה אחת עם חדרים כפולים); ‏`!rq.valid` להרכב → `null` באותו אינדקס; יחידה עם `null` בכל האינדקסים נשמטת. לא נוצרה שום לוגיקת תוספות מחוץ למנוע.

**שומרים.** `check:public-availability-guests` (חדש, 10 בדיקות, fixture של 4 יחידות: ירושה מלאה / חריגה מלאה עם `per_stay` / חריגה חלקית — תינוק מהחדר, מבוגר וילד מהנכס / יחידה קטנה לסינון קיבולת; סכומי הנכס לא-עגולים + עיגול לשקל, כדי שכלל העיגול ייבדק): פרסר; ה-fixture מאומת דרך `resolveEffectivePricing`; תאימות לאחור בלי `guests`; ‏2 מול 4 אורחים על אותה יחידה; שלושת המצבים — ציטוט = מחושב-מהשורות = `calculateReservationPrice` = `priceReservationStays` למבוגרים, ילדים ותינוקות; סינון קיבולת ליחידה; רב-חדרי (`partyPrices` מיושר, `null` לחדר שאינו מתאים, מיון); **המקרה של הבעלים** — חדר 1 = 2+2, חדר 2 = 2+0: היחידה הקטנה מוחזרת עם `null` באינדקס 0 ומחיר מחושב-מה-fixture באינדקס 1, ‏`totalPrice` שלה = אותו מחיר, ויחידה שאינה מתאימה לאף הרכב (3+0 / 2+2) אינה מוחזרת; **הקצאה** — השילוב הזול ביותר שווה ל-brute force עצמאי על תוצאת ה-availability, דירה מועדפת שאינה מארחת את הרכב 1 → `preferred_mismatch`, המלכודת של בחירה חמדנית (INH זולה יותר ל-2+0 אבל רק היא מארחת 2+2 → SMALL לחדר 1, INH לחדר 2), פינים טקסטואליים ש-`createPublicBooking` עובר דרך `assignUnitsToRooms` וזורק `unit_party_mismatch`; שינוי בהגדרות הנכס ובחדר בתוך הטרנזקציה מזיז את הציטוט מיד לערך המחושב מחדש. `check:public-quote` §4 (חדש, §1–§3 ללא שינוי): הציטוט להרכב אמיתי שווה למנוע ולתפר ההזמנה, ועולה על ציטוט 2 המבוגרים בדיוק בסכום המבוגר הנוסף שנקרא משורת הטננט.

**המשך (האתר).** ‏PR 2 ב-sea-tower: `fetchAvailability` מקבלת את ההרכב ושולחת `guests`; ‏`stayTotal` ו-checkout משקפים את כלל ההקצאה (דירה שנבחרה → חדר 1, ואז השילוב התקין הזול ביותר לפי `partyPrices[i]`); אין יותר סינון לפי `maxOccupancy` של הסוג; הודעה לקוד `unit_party_mismatch`.
