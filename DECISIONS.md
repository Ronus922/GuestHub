...
### איפוס הגוף במעבר ל"כתיבת הודעה חדשה" — הוכרע (2026-09-10)

**הכרעת הבעלים:** המעבר ל"כתיבת הודעה חדשה" **מרוקן את גוף ההודעה לגמרי** — תיבה ריקה,
בלי טקסט התבנית ובלי `{{placeholders}}`. המעבר חזרה לתבנית **ממלא מחדש**.
...
**גבול מוצהר — הנושא לא מאופס.** ההכרעה נסחה "גוף ההודעה", וכך יושמה: במייל, שדה **הנושא**
עדיין נושא את נושא התבנית על `{{placeholders}}` שלו אחרי המעבר לכתיבה חדשה (נראה בצילום
`02-custom.png`). המילוי-מחדש כן מחזיר גם אותו. **פתוח להכרעה** אם גם הנושא צריך להתאפס.

**שומר.** `check:send-message-panel` קיבל קבוצה עשירית: 12 טענות runtime על `applyMode`
המקומפל + 4 טענות חיווט. ארבע מוטציות אדוורסריות (ביטול האיפוס, קוד-מת בראש הפונקציה,
...
המעבר: `body=""`, `textarea.value=""`, אפס `{{`, כפתור השליחה **ננעל** והכותרת התחתונה
אמרה "יש למלא את תוכן ההודעה" — ‏`body_empty` מתוך `manualSendGate` האמיתי. החזרה: הגוף
חזר מילה במילה והכפתור נפתח.
