...
## D141 — שאלה פתוחה: פער ה-draft בין חלון "דורש טיפול" לווידג'ט התשלומים (2026-08-08)

**שאלה להכרעת רונן — נרשמת בלי הכרעה ובלי תיקון.**

אחרי #187, שאילתת החובות של חלון "דורש טיפול" (dashboardAlerts, השאילתה
הראשונה ב-data.ts) מחריגה **רק** `wf.key = 'approved'`. ווידג'ט התשלומים
(D139/D140) מחריג **שניים**: `approved` וגם `reservations.status = 'draft'`.
התוצאה: טיוטה עם חוב רשום (total_price > 0, paid < total, לא approved)
מופיעה בחלון "דורש טיפול" כשורת "טרם שולם" — ופעם שנייה שם כשורת אישור —
אבל **נעדרת** מווידג'ט התשלומים. אופרטור שרואה התראת חוב ואז מחפש את ההזמנה
ברשימת החיובים יראה סתירה: המערכת קוראת לזה חוב בחלון אחד וטוענת בחלון
השני שאין מה לחייב.

**השאלה:** האם שאילתת החובות של "דורש טיפול" צריכה להחריג גם `draft`,
כמו הווידג'ט?

**אפשרות א — להחריג `draft` גם שם (יישור מלא ל-D140).** הדוקטרינה נשמרת
עקבית: "טיוטה אינה הזמנה שלא שולמה אלא הזמנה שטרם נוצרה" נכון בשני
החלונות; הטיוטה עדיין מוצגת ב"דורש טיפול" כשורת **אישור** (עם כפתור
draft→confirmed), כלומר לא נעלמת. ההשלכה: חוב שנרשם על טיוטה (מקדמה
חלקית, מחיר שסוכם) לא ייראה כחוב בשום מקום עד שההזמנה תאושר — מי שמסתמך
על התראות הכסף יפגוש אותו רק אחרי האישור.

**אפשרות ב — להשאיר כמות שהוא (מצב #187 הנוכחי).** התראת הכסף שמרנית
ותופסת הכול: חוב על טיוטה נראה מיד. ההשלכה: הסתירה התצוגתית נשארת —
אותה הזמנה "טרם שולם" בחלון אחד ולא-קיימת בשני, בניגוד חזיתי לניסוח D140
("טיוטה אינה הזמנה שלא שולמה"); וטיוטה עם חוב תופסת שתי שורות ב"דורש
טיפול" (כסף + אישור).

הכרעה לאפשרות א' תעדכן את התבנית הקנונית של
`check:alerts-window-includes-cancelled` באותו קומיט (הגארד מקפיא את צורת
השאילתה הנוכחית, והודעת הכשל שלו אומרת זאת במפורש).
...
- פער פתוח (הכרעת רונן): `PRODUCTION_MARKERS` מקבל את ה-DSN הישיר על סמך
  `:5432/` בלבד — ל-IP פנימי של docker אין סימן זהות אמיתי. כל חיזוק שמבטל
  את הספיקות של הפורט מפיל בהכרח את ה-DSN הישיר הזה (נמדד; ראה PR
  fix/liveness-and-grants-hardening). אפשרויות שהועלו: alias יציב/רשומת
  hosts ל-supabase-db שתשמש סימן, או טביעת-אצבע של הקלאסטר
  (`pg_control_system()`) מוצמדת בריפו ונבדקת אחרי חיבור. עד הכרעה —
  הרשימה נשארת כמות שהיא.
