...
### D177 — תוספת: פריסה ואימות חמש ההכרעות על האתר החי (2026-09-07)

**פריסה.** ‏#242 מוזג ל-main ב-merge commit `ab6609e` (הענף נמחק) ונפרס דרך
`PROD_DEPLOY_OK=1 npm run deploy:prod`: build `ObUgKgKH5GqMbaHLC1eHv`, ‏0 מיגרציות ממתינות
(89 קבצים), שני תהליכי PM2 ‏online, ‏`/api/health` 200 ב-loopback ובדומיין, ‏`/` 307 · `/login`
200 · `/calendar` 307. (שורות "Failed to find Server Action" בלוג הן מ-04:51, לפני הפריסה.)

**אימות בדפדפן (magic-link + CDP, טכניקת D173) — 17/17 ‎+ בדיקת רקע נוספת.**
* **§10 ב-390 עם `hasTouch`:** הקשה על "סגור לעזיבה" הדליקה "שינויים שלא נשמרו: כניסה ועזיבה";
  **שלושת מסלולי הסגירה** — Esc, לחיצה על הרקע, "ביטול" — כל אחד העלה את האישור והמגירה נשארה
  פתוחה; "המשך עריכה" החזיר את הפוטר והטיוטה שרדה; "סגור בלי לשמור" סגר, ופתיחה מחדש הציגה את
  המצב המקורי — כלום לא נשמר. **הערה:** ב-390 המגירה במלוא הרוחב ולכן אין פיקסל רקע נגיש, ושם
  הרקע נלחץ תוכניתית; הקשת עכבר **אמיתית** על הרקע הנראה נבדקה ב-1440 (הנקודה 1152,450 פגעה
  ב-`div.absolute.inset-0[aria-hidden]`) והעלתה את האישור.
* **§12:** המגירה מתייצבת ב-**540ms** (הווריאנט המהיר), והרקע הוא `rgba(15, 23, 42, 0.45)` ‎+
  `blur(2px)` — זה של חלונות ההזמנה, כפי שאושר במפורש.
* **§2:** ביום תפוס הצ'יפ קורא "לא זמין למכירה" והמשפט "קיימת הזמנה פעילה ביום זה." — בלי הכפילות.
* **§14 ‎+ תיקון האריחים:** 390 · 780 · 1024 · 1100 → 3×2; 1440 → שישה בשורה; רשת הסנכרון 3
  בשורה בכל הרוחבים; אפס אריחים בודדים, אפס גלישות.

**§4 — מסלול המחיר, כתיבה מאושרת לפרודקשן.** חדר **1131** (`out_of_order`, ‏`numAvail 0` אצל
Beds24 — אי אפשר למכור במחיר החולף), יום **04/08/2027**, שורה שכבר הייתה קיימת עם `price = NULL`:

| שלב | pricing_plan_rates | audit_logs | channel_sync_jobs | Beds24 readback |
|---|---|---|---|---|
| לפני | `price NULL` | — | — | `price1 450` |
| ‏999 ‎+ "שמירת מחיר" | `999.00` | `{"price": 999}` | `sync_ari_range` succeeded ב-411ms | `price1 999` |
| "חזרה למחיר הבסיס" | **`NULL`** (לא 0) | `{"price": null}` | `sync_ari_range` succeeded ב-314ms | **`price1 450`** |

כל שאר עמודות השורה זהות לתמונת ה"לפני" (`stop_sell f`, `min_stay_through 1`,
`min_stay_arrival`/`max_stay` NULL, ‏CTA/CTD `f`) — רק `updated_at` זז, כלומר המצב הוחזר.
‏0 שורות `channel_sync_errors` חדשות (‏50 הפתוחות הן המצבור הישן). ה-readback הוא בדיוק הקריאה
היחידה שמודול `beds24-ari-readback` מתיר (`GET /inventory/rooms/calendar`, קריאה בלבד), הורץ עם
הטוקן שה-worker מתחזק (D138 — בלי הנפקה), שלוש קריאות בעלות קרדיט אחת כל אחת.

**מלכודת אימות שנרשמה:** ‏`.rc-sec-hint` הראשון במגירה שייך לסקשן "מצב מכירה סופי"; ה-hint של
המחיר האפקטיבי הוא זה שבסקשן ששמו "מצב מסחרי". שתי אסרשנים בריצה הראשונה נכשלו על הסלקטור הזה,
לא על המוצר.
