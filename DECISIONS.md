...
## D195 — ‏`/api/public/availability` מתמחר לפי הרכב האורחים (`guests`) דרך המנוע; יחידה שלא מתאימה להרכב אינה מוצעת (2026-09-25)
...
**הכרעות הבעלים (25/09/2026).** (א) שרשרת הירושה — חדר, ואם אין ערך אז הנכס (`resolveEffectivePricing`) — היא מקור האמת לתפוסה ולתוספות; לא משנים אותה ולא נוגעים בנתוני החדרים ב-DB (‏1130/1131 בחריגה חלקית נשארים כפי שהם; הבעלים מעדכן דרך `/rooms` אם ירצה). (ב) כל סכום ב-availability עובר דרך `resolveEffectivePricing` והמנוע, כולל הגדרות הנכס (עיגול, ספירת תינוקות, גילאים) — כך שכל שינוי ב-`/rooms` או בהגדרות הנכס חל באתר מיד. (ג) אין מחיר או תפוסה מקובעים בקוד או בבדיקות. (ד) קודם GuestHub, אחר כך האתר; פריסה לפרודקשן רק באישור.

**החוזה.** `GET /api/public/availability?check_in&check_out[&guests=2-0,3-1]` — `guests` בפורמט האתר: חדר לכל פסיק, `מבוגרים-ילדים` לחדר (מבוגרים 1–6, ילדים 0–4, עד 5 חדרים — אותם גבולות של `POST /api/public/bookings`; הפרסר היחיד: `src/lib/public-booking/guests.ts`). ערך לא תקין → 400 `validation`. עם `guests`: כל יחידה מתומחרת במנוע להרכב הראשון (`totalPrice`), יחידה שאינה יכולה לארח את ההרכב הראשון אינה מוחזרת, ולכל יחידה נוסף `partyPrices` — מערך מיושר לרשימת `guests` (אינדקס 0 ≡ `totalPrice`; ‏`null` = היחידה אינה יכולה לארח את ההרכב של אותו חדר). ‏`availableUnits`, ‏`totalPrice`/`pricePerNight`/`nightly` של הסוג נגזרים מהיחידות המוצעות בלבד; התשובה מהדהדת `guests` בצורתו הקנונית. בלי `guests`: התשובה זהה למה שהייתה (‏2 מבוגרים, הזולה דרך המנוע, השאר ESS; ‏`guests: null`, בלי `partyPrices`) — BIOS Bot ממשיך להרכיב על זה.

**המימוש.** `publicAvailability(db, in, out, { parties })` → `priceUnitsForParties`: קריאת מנוע אחת לכל הרכב **שונה** על כל היחידות הזמינות (המנוע דוחה `roomId` כפול בבקשה אחת — `ROOM_DUPLICATED` — ולכן הזמנה רב-חדרית עם שני הרכבים היא שתי קריאות, לא קריאה אחת עם חדרים כפולים); ‏`!rq.valid` להרכב הראשון → היחידה נשמטת; המיון נשאר מחיר ↑ ואז קוד. `createPublicBooking` קורא לאותה פונקציה עם אותם `parties` (‏`input.rooms`, תינוקות 0) — כך שרשימת היחידות, סדרן וההקצאה המיקומית (`rooms[i] → picked[i]`) הם בדיוק מה שהאתר הציג, ו-`expectedTotal` מושווה לאותו חישוב. לא נוצרה שום לוגיקת תוספות מחוץ למנוע.

**שומרים.** `check:public-availability-guests` (חדש, 8 בדיקות, fixture של 4 יחידות: ירושה מלאה / חריגה מלאה עם `per_stay` / חריגה חלקית — תינוק מהחדר, מבוגר וילד מהנכס / יחידה קטנה לסינון קיבולת; סכומי הנכס לא-עגולים + עיגול לשקל, כדי שכלל העיגול ייבדק): פרסר; ה-fixture מאומת דרך `resolveEffectivePricing`; תאימות לאחור בלי `guests`; ‏2 מול 4 אורחים על אותה יחידה; שלושת המצבים — ציטוט = מחושב-מהשורות = `calculateReservationPrice` = `priceReservationStays` למבוגרים, ילדים ותינוקות; סינון קיבולת ליחידה; רב-חדרי (`partyPrices` מיושר, `null` לחדר שאינו מתאים, מיון); שינוי בהגדרות הנכס ובחדר בתוך הטרנזקציה מזיז את הציטוט מיד לערך המחושב מחדש. `check:public-quote` §4 (חדש, §1–§3 ללא שינוי): הציטוט להרכב אמיתי שווה למנוע ולתפר ההזמנה, ועולה על ציטוט 2 המבוגרים בדיוק בסכום המבוגר הנוסף שנקרא משורת הטננט.

**המשך (האתר).** ‏PR 2 ב-sea-tower: `fetchAvailability` מקבלת את ההרכב ושולחת `guests`; ‏`stayTotal` ו-checkout משתמשים ב-`partyPrices[i]` לחדר ה-i; אין יותר סינון לפי `maxOccupancy` של הסוג.
