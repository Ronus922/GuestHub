...
`ari_readback_commercial` ("פער מסחרי, לא סכנת double-booking"); השאר →
`ari_readback_drift`. **קוד אחד והודעה אחת למחזור, לפי הגרוע ביותר שנמצא** —
מחזור שמחזיק את שניהם הוא מחזור overbooking, והפיצול המדויק תמיד נוסע ב-`context`.
