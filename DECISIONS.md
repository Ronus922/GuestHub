...
## D98 — השהיית-קרדיט לעולם לא בולעת כישלון אמיתי: dead-letter נשמר

**מה נמצא.** מדיניות D97 ("השהיית-קרדיט אינה כישלון טווח") מומשה ב-`drainBeds24AriDirtyRanges`
בתנאי `if (creditPause && warnings.length === 0)`. אבל **כל** תשובה מוזנת לשער — גם תשובה
כושלת. `sendCalendarRequests` קורא ל-`gate.observe(res.credits, …)` לפני שהוא בודק `!res.ok`,
ולכן תשובת 500/400/network שבמקרה נשאה `x-five-min-limit-remaining` מתחת לסף הפעילה
`creditPause` ונכנסה לענף ההשהיה. התוצאה: `attempts` לא עלה, `last_error_code` נשאר NULL,
`channel_connections.last_error` נשאר NULL, וה-ledger תיעד `partial / credit_paused` במקום
`failed / server_error`. **טווח שלעולם לא יצליח מפסיק להגיע ל-`max_attempts` — הוא חוזר לנצח
ושורף בדיוק את הקרדיטים שההשהיה נועדה להגן עליהם.** זו רגרסיה מול הקוד שלפני D97.

**הראיה (staging :5434, דרך מודולי ה-dist האמיתיים, טווח מלוכלך אחד, `max_attempts=5`):**

| תרחיש | main (לפני D97) | D97 כפי שנכתב | אחרי D98 |
|-------|------------------|----------------|-----------|
| 500 + remaining=8.4, drain אחד | attempts=1, `server_error`, `failed` | **attempts=0, NULL, `credit_paused`** | attempts=1, `server_error`, `failed` |
| 400 קבוע + remaining=8.4, 8 מעברי scheduler | `status='failed'` אחרי 5 קריאות | **`status='pending'`, attempts=0, 8 קריאות** | `status='failed'` אחרי 5 קריאות |

**ההחלטה.** ההשהיה מחליפה את מסלול הכישלון **רק כשחלון הקרדיטים הוא כל הסיפור**: או שדבר לא
נכשל (עצרנו מרצוננו לפני הקיר), או שהכישלון **הוא** ה-429 שההשהיה מתארת. כל כישלון ספק אחר —
500, 400, רשת, אימות — נשאר כישלון: הוא מחייב ניסיון ורושם `last_error`. כדי לא לוותר על
הקצב, כשמד הקרדיטים היה נמוך **גם** הצינון של ה-breaker נלקח מ-`resets-in` של הספק ולא
מההשהיה האקספוננציאלית — הטווח מחויב בניסיון **וגם** החיבור נעצר לחלון.

**השומר.** `check:beds24-credit-backoff` היה ירוק גם אחרי **מחיקה מלאה** של שער הקרדיטים
היוצא — הוא כיסה רק את המודול הטהור ואת מסלול המשיכה הנכנס. נוספו ארבע טענות DB-backed דרך
`drainBeds24AriDirtyRanges` האמיתי: חלון בריא דוחף ומסנכרן; remaining נמוך על תשובה **מוצלחת**
דורך מחדש את הטווח בלי ניסיון ומחזיק את ה-breaker ל-`resets-in`; 500 על מד נמוך **כן** מחייב
ניסיון ורושם `server_error`; וטווח כושל קבוע מגיע ל-dead-letter ב-`max_attempts`. הטענות
המבניות סומנו במפורש `CONTRACT` עם הודעת כשל שאומרת שזו הפרת חוזה ולא הפרת התנהגות.
