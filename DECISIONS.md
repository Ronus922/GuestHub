...
## D98 — איפוס maxStay ל"ללא הגבלה" נחסם פעמיים: אין מסלול UI ל-NULL, והשמטה על החוט אינה מנקה

**המשימה (2026-07-25).** להעביר `maximum לילות` ל"ללא הגבלה" (NULL) בכל החדרים, מהיום ולהבא. **לא בוצעה.** שני חסמים בלתי-תלויים, שניהם מבוססי מדידה — הפירוט המלא ב-`docs/RESET_MAX_STAY.md`.

**חסם 1 — סמנטיקת החוט אינה משיגה את היעד.** `beds24-ari-payloads.ts:134,238` **משמיטים** את `maxStay` כשהערך NULL — לא שולחים 0 ולא מחרוזת ריקה, כך שאין סכנת חסימה. אבל `POST /inventory/rooms/calendar` הוא **עדכון חלקי**: מה שלא נשלח אינו נמחק. נמדד חי על שלושה חדרים — 1329 (שידרנו 31) מציג 31; 1318 ו-1042 (מעולם לא שידרנו) מציגים **365**, ברירת המחדל ברמת החדר בפאנל. חיזוק עצמאי: 1042 מציג `price1=593.18` בעוד המחיר המקומי 700 ואיננו שולחים לו מחיר כלל — ערך ששרד משליחה קודמת. **מסקנה: כתיבת NULL הייתה מעבירה 4,548 שורות ל-NULL בלוח שלנו בזמן ש-Beds24 ממשיך לאכוף 31 — סטייה שקטה בין המערכת לערוץ שאינה קיימת היום.**

**חסם 2 — אין מסלול כתיבה מורשה בהיקף.** חוק הברזל של המשימה מחייב מעבר דרך `bulkUpdateRatesAction`; זו Server Action שפותחת ב-`getActor()` (`auth/actor.ts:80-85`) ולכן מחייבת session מאומת מתוך cookies — לא ניתנת להפעלה מתהליך רקע. ובמסך עצמו, ה-`Stepper` של העדכון הקבוצתי (`GroupUpdatePanel.tsx:517-527`) מוגדר `value: number` וחסום ב-`Math.max(0, …)`: **גם מפעיל אנושי אינו יכול לבטא NULL שם.** NULL כן נגיש — אך רק תא-אחר-תא ב-`CellDetailPanel.tsx:243`, כלומר 4,548 עריכות ידניות. הסכימה עצמה תומכת (`validation/rates.ts:7` הוא `.nullable()`) — **החסר הוא ב-UI בלבד.**

**מקור ה-31 — נשלל כברירת מחדל של קוד.** אין `DEFAULT` בסכימה (מיגרציות 009/016), ברירת המחדל בטופס היא 7, ואין 31 באף מיגרציה או seed. הוא נכתב ידנית ע"י מפעיל בשני עדכונים קבוצתיים: `bulk_rate_update_logs` `70f538fe` (05/07, 14 יחידות, 2026-07-05→2027-08-03) ו-`1a22ab62` (19/07). התפלגות: 4,788 שורות עם 31, 2,686 NULL, 12 עם 7. **1318 ו-1042 הם 100% NULL** — הנחת "31 בכל החדרים" שגויה לגביהם.

**ממצא נלווה.** `stayField` מתיר `min(0)` (`validation/rates.ts:7`) בעוד `validateBeds24CalendarRequest` דורש `>= 1` (`beds24-ari-payloads.ts:288`). ב-2026-07-19 08:10:39 מפעיל אכן שלח `maxStay: 0` על 14 יחידות ותיקן דקה אחר כך; ערך כזה מפיל את **כל** מטען ה-ARI כ-`validation`, לא רק את השדה הפוגע.

**מה נדרש (ענף LOS, לא בוצע):** מצב "ללא הגבלה" ב-Stepper של העדכון הקבוצתי שמשדר `null`; `stayField` ל-`min(1)`; והכרעה אם "ללא הגבלה" מול Beds24 פירושו שידור מפורש (למשל 365) במקום השמטה. בנוסף — החלטת מפעיל על `maxStay` ברמת החדר בפאנל Beds24 (365), שבו לא נגעתי.

**צילום החזרה** של 4,548 השורות המועמדות נשמר ב-`docs/RESET_MAX_STAY_SNAPSHOT.csv` כדי שהשינוי יהיה בר-ביצוע מיידית ברגע שהחסמים ייפתרו.
