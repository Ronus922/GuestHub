...
* **Continuous run mode (no inter-stage checkpoint).** There is no user checkpoint between stages. When a stage's exit gate has fully passed (State file updated, stage report written, tag created, draft PR updated), proceed **immediately, in the same session,** to the next stage's entry gate without waiting for user input, and keep going until the `stage-7-complete` tag exists. Within a stage, execute without routine questions per V2 §2.
  * **"Fresh session per stage" is a disk-discipline rule, NOT a stop signal.** It means only this: at every stage entry, re-read this charter, `GUESTHUB_PROGRAM_V2.md`, the stage document, `docs/program/STATE.md` and the previous stage report *from disk*, and rely on those files (never in-conversation memory) as the source of truth for what to do next. It does **not** mean end the turn, hand off, or wait for a human between stages. Context-window limits are handled by the State-file handoff, not by pausing: keep executing; if a context boundary is actually reached mid-work, the next invocation resumes from STATE — but never stop *voluntarily* at a stage boundary.
  * Stop only for a genuine external blocker as defined in V2 §2 (a missing credential, an unprovisionable paid account, absent DNS access, or an irreversible live-production action); even then, document the blocker in the State file, complete everything else possible, and continue. Finishing a milestone and reporting status is not a reason to end the turn — complete the milestone, run its check, commit, update STATE, and move straight to the next. The absolute safety boundaries of V2 §3 remain binding at all times.
