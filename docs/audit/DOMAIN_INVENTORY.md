...
| ~~`payment_policies`~~ / ~~`payment_policy_stages`~~ | — | **Removed** (migration 078): superseded by direct payment-method management on `lookup_items` category=`payment_methods`. | — |
...
| 10 | **Low** | `lookup_items` polymorphism is not FK-safe: `reservations.workflow_status_id` FK does not enforce `category='workflow_statuses'` (any lookup row is accepted at DB level), and `payments.method` references payment-method keys as text with no referential check. | 030 §2 |
