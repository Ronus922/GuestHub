...
| P-23 | `CellDetailPanel.tsx` (`widthClassName="rc-panel w-full md:w-[85%] lg:w-[60%]"`) | הסולם הקנוני של `SidePanel` — מלא <768 · 85% ‏768–1023 · 60% ≥1024 (D177 §14) | |
...
# סבב 4 — פאנל עריכת ההזמנה (2026-08-13)

הסבב הזה לא התחיל ממדידה אלא מ**צילומי מכשיר אמיתי**: גוף ההזמנה מוסט הצידה
מחוץ למסך, ופס גלילה דק לצד הטופס. שני התסמינים שוחזרו במדידה לפני כל שינוי קוד.

## פריטים שהיו PASS ונמצאו שבורים

| פריט | היה | נמדד בפועל | עכשיו |
|---|---|---|---|
| ‏P-02 `EditReservationPanel` — פריסה במובייל | PASS (‏נמדד ברמת ה-route בלבד) | ‏183 אלמנטים מחוץ ל-viewport ב-390; `.bw-col-main` ‏443px ב-left:-79 | **FIXED + PASS** |
| ‏K — פוטר המגירה (`.dw-ft`) | PASS | ‏"בטל הזמנה" ב-left:272 → right:415 בתוך 390 | **FIXED + PASS** |
| ‏P — תפריט הפעולות בכותרת (`.bk-tb-menu`) | FIXED + PASS (‏תקרת רוחב) | ‏left:-90 ב-320 · left:-22 ב-390 — התקרה לא מנעה יציאה מהמסך | **FIXED + PASS** |
| ‏G — `.thin-scroll` במגע | לא נבדק | פס גלילה קלאסי קבוע במקום מחוון הפלטפורמה | **FIXED + PASS** |
| ‏O-1 מסבב 1 — חפיפת אזורי מגע | PASS (‏0 החטאות) | ‏7 חפיפות ב-/staff בבדיקת `elementFromPoint` | **FIXED + PASS** |

**למה זה חמק:** מטריצת ה-routes מודדת מסך שנטען, לא פאנל שנפתח בתוכו. הפאנל
נמדד ברמת המסלול (‏R-05 `/reservations`), והפגם חי רק אחרי לחיצה על שורה. הכיסוי
החדש פותח אותו בפועל ומודד 18 מצבים בכל viewport.

## מה נמדד בסבב הזה

| מדידה | תוצאה |
|---|---|
| מצבי פאנל עריכה (‏3 vp × 18) | **54 עברו · 0 נכשלו · 0 לא-נמדדו** |
| סקרולרים אנכיים בגוף הפאנל | **1 בדיוק** בכל 54 המצבים (‏0 מקוננים) |
| מטריצה ראשית | ‏391/391 תאים · **0 P0** · 0 גלישה אופקית · 0 שגיאות ניווט/JS/probe |
| הוכחת רינדור (חדש) | **391/391** · **0 error-boundary** |
| בדיקת פגיעה אמיתית | ‏12 מסלולים · **0 מתחת ל-44** · **0 חפיפות** (היו 7) |
| מטריצת מגע | ‏0 P0 |
| משטחים צפים ב-390×844 | ‏20/21 נפתחו · 0 רחבים/גבוהים מה-viewport · 0 כשלי Escape · 0 פוטר מחוץ למסך |
| ‏`check:responsive` | ‏14 → **18** |

## פריטים חדשים ב-OPEN / HUMAN REVIEW

| מזהה | סטטוס | פירוט |
|---|---|---|
| ‏P-20 · drawer הקודים ב-`/locks` | **BLOCKED** | ‏0 שורות `ttlock_locks` ב-staging. ללא שינוי מסבב 3 |
| ‏O-1 · WebKit | **BLOCKED** | דורש `sudo` על מארח משותף שמריץ פרודקשן |
| ‏P-45 · מגירת הניווט במובייל | **HUMAN REVIEW** | נמדד `close=N lock=N` — נסגרת ב-Escape וב-backdrop, אך אין כפתור סגירה מפורש ואין נעילת גלילה. לא P0, החלטת עיצוב |
| ‏/housekeeping/my-tasks | **HUMAN REVIEW** | ‏17 תאים רנדרו תוכן אמיתי בלי אף פקד אינטראקטיבי בתוך `<main>` — מצב ריק לגיטימי בצפיפות ה-staging, לא דף שגיאה |
...
| PASS | 90 | **85** |
| FIXED + PASS | 63 | **68** |
| OPEN / BLOCKED | 1 | **1** (‏P-20) |
| HUMAN REVIEW | — | **2** |
| NOT APPLICABLE | 1 | 1 |
| **סה"כ** | **155** | **157** |

חמישה פריטים עברו מ-PASS ל-FIXED + PASS מאותה סיבה כמו בסבב 3: הם היו PASS על
סמך מדידה שלא פתחה את המשטח שבו הפגם חי.

---

# סבב 5 — סנכרון מול `main` עדכני (2026-08-13)

הענף מוזג עם `origin/main` @ `946872d` (‏#197). הסבב הזה **אינו מוסיף תיקונים** —
הוא מודד מחדש את כל מה שסבב 4 מדד, על עץ שכבר אינו אותו עץ.

## שינויי סטטוס

| פריט | סבב 4 | סבב 5 | למה |
|---|---|---|---|
| ‏O-8 · בניית worktree מפיקה CSS חסר | OPEN (מלכודת ידועה) | **נסגר — לא משוחזר** | שני העצים נבנו `exit 0` עם צ'אנק CSS מלא. התסמין נגרם מ-`next dev` שרץ על אותו `.next`; עצירת שרת ה-dev לפני `pnpm build` מסירה אותו לגמרי |

אף פריט לא נסוג. `P-20`, `O-1` (‏WebKit), `P-45` ו-`/housekeeping/my-tasks` ללא שינוי.

## מה נמדד מחדש (הכול על העץ הממוזג)

| מדידה | תוצאה | מול סבב 4 |
|---|---|---|
| מטריצה ראשית ‏23×17 | ‏391/391 · 0 P0 · 0 גלישה אופקית · 0 שגיאות | זהה |
| הוכחת רינדור | ‏391/391 · 0 error-boundary | זהה |
| מטריצת מגע ‏22×3 | ‏66 תאים · 0 P0 · 0 INPUT-ZOOM | זהה תו-בתו |
| בדיקת פגיעה | ‏0 מתחת ל-44 · 0 חפיפות | זהה |
| משטחים צפים | ‏20/21 · 0 כשלי Escape · 0 פוטר מחוץ למסך | זהה |
| חסימת חדר במובייל | ‏ALL CHECKS PASSED (כולל שמירה בפועל) | זהה |
| לוח תעריפים בגובה נמוך | ‏8 / 6 / 6 / 12 שורות גלויות | זהה |
| ‏R-24 מסלול הדפסה | ‏17 תאים · 0 P0 · 0 P1 | נמדד עם מזהה אמיתי |
| פאנל עריכת ההזמנה | **72** מצבים (‏4 vp × 18) · 0 כשלים | הורחב מ-54 |
| ‏`check:responsive` | ‏18/18 | זהה |
| ‏suite | ‏86/8/1 מ-95 מול 85/8/1 מ-94 ב-main נקי — **0 רגרסיות** | בייסליין חדש |
...
| PASS | 85 | **85** |
| FIXED + PASS | 68 | **68** |
| OPEN / BLOCKED | 1 (‏P-20) | **1** (‏P-20) |
| HUMAN REVIEW | 2 | **2** |
| NOT APPLICABLE | 1 | 1 |
| **סה"כ** | **157** | **157** |

הספירה לא זזה במכוון: סבב 5 הוא אימות, לא סבב תיקונים.
