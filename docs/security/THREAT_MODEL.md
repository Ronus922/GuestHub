...
4. **External ↔ webhook endpoints** — unauthenticated server-to-server POSTs (Beds24, Twilio, GREEN-API) authenticated by opaque path token (+ Twilio HMAC).
5. **Next server ↔ third-party APIs** — outbound fetches to Google OAuth/Gmail, Twilio, GREEN-API, Beds24, using operator-configured hosts/credentials.
...
- **Gaps:** `console.error("[reservation-cards]", e)` (`src/app/(dashboard)/reservations/card-actions.ts:53`) logs whole error objects — low risk of provider metadata in logs, no PAN observed. **Severity: Low.**

### Asset C — Channel & messaging credentials (Beds24 API key, Gmail refresh token, Twilio/GREEN-API secrets)
...
| F4 | **GREEN-API webhook token plaintext at rest & sole authenticator** (no provider signature); inconsistent with the Beds24 hashed-token model | Medium | `src/lib/messaging/store.ts:83`; contrast `src/app/api/channel/webhook/[token]/route.ts:56` |
...
| F8 | **Error-object logging** may capture provider metadata (no PAN/CVV observed) | Low | `card-actions.ts:53` |
...
- Card vault fail-closed AES-256-GCM; **CVV never persisted** (migration 018); Twilio HMAC timing-safe; Gmail OAuth CSRF state + re-auth; SSE endpoint auth-checked and tenant-scoped at connect (`src/app/api/events/route.ts:26-45`); webhook dedupe/idempotency across Beds24 + Twilio.
