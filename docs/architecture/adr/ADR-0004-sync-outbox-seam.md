...
canonical domain write (Stage 3 services)
  └─ same transaction → markAriDirty(connection, room, rate?, date-range)  →  channel_dirty_ranges   (the OUTBOX)
                                                                                      │
worker drain (drainAriDirtyRanges) ── coalesce ranges ── build ONE batched payload ── pushAri() ── Beds24
                                                                                      │
...
1. **Transactional dirty-range marking is the only way ARI change enters the outbox.** No worker or UI path pushes to Beds24 outside this seam (satisfies pre-flight items 1–2; keeps the "delete all cert code and it still works" property).
...
- Stage 4 builds: the Beds24 wiring on top (single sync envelope, Group Update expansion, evidence ledger, cooldown/circuit-breaker, Min Stay declaration, booking-receiving flow).
- Because the seam already exists and is anti-pattern-free, this is hardening, not a rewrite — low regression risk, and it preserves the confirmed strength "the channel-sync architecture is well-formed".
