...
| P0-3 | **Calendar read-back reconciliation**: periodic `GET /inventory/rooms/calendar` diffed against our projection; alert on drift. (`reconcile_inventory` is no longer dormant — D93 wired it for BOOKING reconciliation, 20-min cadence; ARI read-back would extend that same job, not a new type) | Today nothing detects Beds24 holding stale ARI (the overbooking recipe); read-only API call | M | low — read-only; credit cost must be measured (X-RequestCost) |
