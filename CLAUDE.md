...
> **כלל ברזל 12 — אין החלפה שקטה של החלטה של הבעלים:** כשהוראה מפורשת נראית לך שגויה, לא-אופטימלית או מיותרת — **עצור, נמק, וחכה לאישור** לפני שאתה מבצע אלטרנטיבה. אסור לבחור באפשרות ב' כי היא "נכונה יותר", גם כשאתה צודק לגופו של עניין וגם כשהבעלים היה מאשר אילו נשאל: הוראה שהוחלפה בשקט גוזלת מהבעלים את ההכרעה **ואת הידיעה שהייתה כאן הכרעה**. הנוסח: "ביקשת א'. לדעתי ג' עדיף כי X. להמשיך עם א' או לעבור ל-ג'?" — ואז לחכות. **וכשמתעדים החלטה כזו: לא לכתוב "החלטת בעלים" על משהו שהבעלים לא הכריע בו.** תקדים: D159 — ההנחיה הייתה לתקן את 6 הפרות ה-design, ובמקומה נוצר baseline; אישור בדיעבד אינו הופך את הדרך לתקינה. הבית הקנוני: AGENTS.md כלל 12; `check:agents-rule12` מאדים אם הכלל נמחק מאחד משני הקבצים.
...
> הבית הקנוני של הסעיף הזה הוא CLAUDE.md (בריפו). העותק ב-AGENTS.md נמחק ברגנרציית `gen-catalog.sh` ב-22/07/2026 מפני שהסעיף היה קיים אז **רק** ב-AGENTS.md, והרגנרציה בונה את AGENTS.md מגוף ה-CLAUDE.md (D90; המנגנון הוכרע ב-D193). `check:agents-concurrency` מתריע אם זה קורה שוב.
