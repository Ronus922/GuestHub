...
## 13. כשלים נושאים את הראיות של עצמם (D112 — כלל קבוע לכל עבודה עתידית)

מסלול כשל שמדווח **קטגוריה** בלי לשמר את **תשובת הספק הגולמית** הוא דפקט, באותה
דרגה כמו בליעה שקטה של שגיאה. אבחון לעולם לא ידרוש להריץ את הכשל שוב כדי לגלות
מה נאמר.

לכל כשל מול ספק חיצוני, ברגע הכשל ולפני כל parsing/מיפוי/קטגוריזציה:

1. **סטטוס HTTP מילולי** כפי שהתקבל. לא הגיעה תשובה → NULL מפורש, לעולם לא
   מספר מומצא.
2. **גוף התשובה הגולמי, ללא שינוי**, קטום ל-2KB עם סמן קטיעה מפורש.
3. **המטען ששלחנו** (או הפניה יציבה אליו).
4. **חותמת UTC** של רגע התצפית.

הקטגוריה הממופה היא נתון **נגזר** — מותר לשמור אותה לצד המקור, אסור שתחליף אותו.
...
## 16. חוקי @layer — כניעות זה שכבה, לא ספציפיות (D148, כלל קבוע)

1. **הצהרה לא-משוכבת גוברת על כל `@layer`** — ללא קשר לספציפיות. ‏`:where()` מאפס ספציפיות בלבד; הוא **לא** מגן מפני כלל layered, כי ספציפיות נשקלת רק בתוך אותה שכבה.
2. כלל גלובלי שנועד **לא לנצח** (רמז, ברירת מחדל, הרחבת מגע) חייב לשבת בשכבה מוקדמת — `@layer base` — כך שכל הצהרה מפורשת (‏components / ‏utilities / ‏partial לא-משוכב) גוברת עליו.
3. כלל שחייב **לנצח** partial לא-משוכב (כמו row→card של המובייל) נשאר לא-משוכב בכוונה — ומתועד במקום, עם הגארד ששומר עליו.
4. אסור `position:` לא-משוכב תחת `@media (pointer: coarse)` — זה בדיוק מה שהפך את רצועות היומן במראה על כל מכשירי המגע. שומר: ‏`check:responsive #20`.
5. **אמולציית מובייל בלי מגע היא לא אמולציית מובייל:** כל בדיקת דפדפן למובייל חייבת `hasTouch: true`/פרופיל מכשיר כך ש-`(pointer: coarse)` מתקיים — צמצום רוחב בלבד מחמיץ את כל כללי המגע. הסיפור המלא: ‏DECISIONS.md ‏D148.
