...
// ---------- DB half ----------
// The read models are exercised on the guard's OWN tenant — one room type, two
// rooms, two one-room Sellable Units and their base plans — built inside a
// transaction that ROLLS BACK; every scenario below runs in a savepoint on top
// of that baseline. Nothing is read from live rows: the suite hands each
// DB-backed guard an empty clone of the from-zero template, where the former
...
const rolledBack = (e) => { if (e.message !== "ROLLBACK") throw e; };

try {
  await sql.begin(async (db) => {
    const [{ id: tenantId }] = await db`
      INSERT INTO guesthub.tenants (name, slug)
      VALUES ('effective-state-check', ${"ess-" + crypto.randomUUID().slice(0, 8)}) RETURNING id`;
    const [rt] = await db`
      INSERT INTO guesthub.room_types (tenant_id, name, base_price)
      VALUES (${tenantId}, 'effective-state type', 500) RETURNING id`;
    for (const code of ["ESS-1", "ESS-2"]) {
      const [room] = await db`
        INSERT INTO guesthub.rooms (tenant_id, room_number, room_type_id, status, is_active)
        VALUES (${tenantId}, ${code}, ${rt.id}, 'available', true) RETURNING id`;
      const [su] = await db`
        INSERT INTO guesthub.sellable_units (tenant_id, code, name, room_type_id, is_pooled, is_active)
        VALUES (${tenantId}, ${code}, ${code}, ${rt.id}, false, true) RETURNING id`;
      await db`
      ...
      await db`
        ...
        VALUES (${tenantId}, ${su.id}, 'base', 'base', true)`;
    }
    ...
    const [{ n: availMismatch }] = await db`
    ...
    const [{ n: priceMismatch }] = await db`
      SELECT count(*)::int AS n
      FROM guesthub.effective_sell_state(${tenantId}, ${FROM}, ${TO}) e
