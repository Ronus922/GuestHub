...
if (paidDrift==="0") ok("paid_amount == SUM(paid payments) for every reservation");
else bad("paid_amount drift", paidDrift);
...
if (balDrift==="0") ok("balance == total_price - paid_amount (unfloored)");
else bad("balance drift", balDrift);
...
if (badStatus==="0") ok("all payment statuses in the canonical set");
else bad("unknown payment status", badStatus);
...
if (orphan==="0") ok("no orphan payments");
else bad("orphan payments", orphan);
...
if (dupIdem==="0") ok("payment idempotency keys unique per tenant");
else bad("duplicate idempotency keys", dupIdem);
